...
mod grouped {
    ...
    pub fn missing_run_error(run_id: &str) -> Error {
    ...
    pub fn missing_run_guidance(run_id: &str) -> Vec<String> {
    ...
    pub fn missing_run_guidance_for_runner_ids(
        run_id: &str,
    ...
    pub fn public_url_for_url_artifact(artifact: &ArtifactRecord) -> Option<String> {
    ...
    pub fn classify_persisted_artifact(
    ...
    pub fn apply_persisted_artifact_cleanup(
        store: &ObservationStore,
        artifact: &ArtifactRecord,
        artifact_root: &Path,
    ) -> Result<()> {
    ...
    pub fn persisted_artifact_path_from_record(artifact_root: &Path, raw: &str) -> PathBuf {
    ...
    pub fn path_is_within_root(path: &Path, artifact_root: &Path) -> bool {
        if path
            .components()
    ...
    pub fn path_size_bytes(path: &Path, metadata: &fs::Metadata) -> Result<u64> {
        if metadata.is_dir() {
            let mut total = 0;
            for entry in
    ...
    pub fn persisted_artifact_read_dir_error(path: &Path, err: io::Error) -> Error {
        Error::internal_io(
            err.to_string(),
            Some(format!(
    ...
    pub fn persisted_artifact_remove_error(path: &Path, err: io::Error) -> Error {
        Error::internal_io(
            err.to_string(),
    ...
    pub struct RunnerDownloadCleanupPreview {
        file_count: usize,
    ...
    pub fn runner_download_root(runner: Option<&str>, run_id: Option<&str>) -> Result<PathBuf> {
    ...
    pub fn cleanup_path_component(name: &str, value: Option<&str>) -> Result<Option<String>> {
    ...
    pub fn plan_runner_download_cleanup(root: &Path) -> Result<RunnerDownloadCleanupPreview> {
    ...
    pub fn collect_runner_download_cleanup(
        root: &Path,
        path: &Path,
    ...
    pub fn runner_cache_directory_error(path: &Path, err: io::Error) -> Error {
        Error::internal_io(
            err.to_string(),
            Some(format!(
    ...
    pub fn remove_runner_download_root(root: &Path) -> Result<()> {
        let metadata = fs::symlink_metadata(root).map_err(|err| {
            Error::internal_io(
                err.to_string(),
                Some(format!("read runner artifact cache {}", root.display())),
    ...
    pub fn relative_cleanup_path(root: &Path, path: &Path) -> String {
    ...
    mod tests {
        ...
        fn missing_run_guidance_prints_runner_routed_retrieval_commands() {
            let hints =
                missing_run_guidance_for_runner_ids("run-123", vec!["homeboy-lab".to_string()]);
            assert_eq!(
                hints,
                vec![

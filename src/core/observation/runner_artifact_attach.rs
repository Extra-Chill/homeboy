...
use crate::core::runners::{Runner, RunnerKind};
use crate::core::{server, Error};
...
pub fn copy_runner_artifact_source(
    runner: &Runner,
    path: &str,
) -> crate::core::Result<RunnerAttachSource> {
    match runner.kind {
        RunnerKind::Local => {
...
fn remote_runner_artifact_type(
    ...
    path: &str,
) -> crate::core::Result<RunnerAttachArtifactType> {
...
fn attach_download_path(runner_id: &str, path: &str) -> crate::core::Result<PathBuf> {
    let file_name = Path::new(path)
        .file_name()
        .and_then(|name| name.to_str())
        .filter(|name| !name.is_empty())
        .unwrap_or("artifact");
    Ok(crate::core::artifact_root()?
        .join("runner-attach")
        .join(runner_id)

...
fn resolve_run_label(store: &ObservationStore, label: &str) -> Result<Option<RunRecord>> {
    ...
    {
        let Ok(data) = crate::core::runners::daemon_api_get(&report.runner_id, "/runs?limit=200")
        else {
            continue;
        };
...
fn finish_stale_runner_child_run(store: &ObservationStore, job: &crate::core::runners::RunnerJob) {

...
mod grouped {
    ...
    pub struct TraceRunOutput {
        ...
        #[serde(skip_serializing_if = "Option::is_none")]
        pub failure: Option<super::super::run::TraceRunFailure>,
        #[serde(default, skip_serializing_if = "Vec::is_empty")]
        ...
        #[serde(skip_serializing_if = "Option::is_none")]
        pub toolchain: Option<super::super::parsing::TraceToolchainProvenance>,
        #[serde(skip_serializing_if = "Option::is_none")]
        pub components: Option<super::super::parsing::TraceComponentsProvenance>,
    }
    ...
    pub struct TraceListOutput {
        ...
        pub count: usize,
        pub scenarios: Vec<super::super::parsing::TraceScenario>,
        #[serde(default, skip_serializing_if = "Vec::is_empty")]
    ...
    pub fn is_default_usize(value: &usize) -> bool {
        value.eq(&usize::default())
    }

    pub fn is_default_bool(value: &bool) -> bool {
        !*value
    }
    ...
    pub fn from_run_workflow_result(
        result: TraceRunWorkflowResult,
        rig_state: Option<RigStateSnapshot>,
    ) -> TraceCommandOutput {
    ...
    pub fn push_toolchain_markdown(out: &mut String, results: &TraceResults) {
    ...
    pub fn push_git_provenance_line(
        out: &mut String,
        label: &str,
        provenance: &super::super::parsing::TraceGitProvenance,
    ) {
    ...
    pub fn reportable_trace_artifacts(artifacts: &[TraceArtifact]) -> Vec<TraceArtifact> {
        artifacts
            .iter()
    ...
    pub fn push_omitted_detail_line(
        out: &mut String,
        label: &str,
    ...
    pub fn push_indented_omitted_detail_line(
        out: &mut String,
        label: &str,
        metadata: &crate::core::engine::detail_output::DetailOutputMetadata,
    ) {
    ...
    mod tests {
        ...
        fn test_from_main_workflow_outputs_keeps_full_output_for_summary_artifact() {
            ...
            let stdout_value =
                serde_json::to_value(stdout_output).expect("summary should serialize");
            let artifact_value =
                serde_json::to_value(artifact_output.expect("full artifact output"))
        ...
        fn test_render_markdown() {
            ...
            assert!(markdown.contains(
                "- **Homeboy:** `/repo/homeboy` @ `abc123` (branch `main`, dirty `false`)"
            ));
            assert!(markdown.contains(
                "- **Target:** `/repo/studio` @ `def456` (branch `trunk`, dirty `false`)"
            ));
            assert!(
                markdown.contains("| `submit_to_cli` | `ui.submit` | `cli.start` | 42ms | ok |")
            );
        }

...
pub struct BenchResults {
    ...
    /// Homeboy-derived phase rollup computed from `phase_events`.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub phase_summaries: Vec<BenchPhaseSummary>,
    /// Homeboy-derived classification for the first timeout/failure phase.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct BenchPhaseEvent {
    /// Generic phase identifier. Homeboy treats this as a label, not a closed
    /// enum, so extensions can model their own lifecycle without core-specific
    /// semantics.
    pub phase: String,
...
mod tests {
    ...
    fn derives_phase_summaries_and_timeout_classification() {
        let raw = r#"{
            ...
            "phase_events": [
                ...
                { "phase": "dependency_preparation", "status": "heartbeat", "t_ms": 1000, "message": "installing" },
                {
                    ...
                    "status": "timeout",
                    "t_ms": 2000,
                    "message": "npm install exceeded budget",
        ...
        assert_eq!(
            ...
            Some("npm install exceeded budget")
        );
    }

...
use crate::core::agent_task::{AgentTaskArtifact, AgentTaskDiagnostic, AgentTaskOutcome};
...
impl ExecutorArtifactRootIdentity {
    pub(crate) fn capture(path: &Path) -> Result<Self> {
        ...
        let finalized_path = crate::core::paths::artifact_root()?
        ...
        for artifact in &mut outcome.artifacts {
            ...
            match staged_file(root, artifact, &declared) {
                ...
                Err(error) if is_legacy_outside_root(&error) => {
                    ...
                    artifact.mime = crate::core::artifact_metadata::content_type_from_path(
                        Path::new(&declared),
                    );
                    ensure_metadata(artifact)
...
fn staged_file(
    ...
    match fs::hard_link(&temporary, &destination) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
            let existing_size = fs::metadata(&destination)
                ...
                .len();
            let existing_sha256 = crate::core::artifact_metadata::sha256_file(&destination)?;
            fs::remove_file(&temporary).map_err(|error| {
                Error::internal_io(
                    error.to_string(),
                    Some("remove finalized artifact staging file".to_string()),
    ...
    Ok((
        ...
        size,
        sha256,
        crate::core::artifact_metadata::content_type_from_path(&destination),
    ))
}
...
mod tests {
    ...
    fn finalization_persists_opened_bytes_in_canonical_store_with_hash_parity() {
        with_isolated_home(|home| {
            ...
            assert!(persisted.starts_with(crate::core::paths::artifact_root().expect("store root")));
            assert_eq!(
            ...
            assert_eq!(
                artifact.sha256,
                Some(
                    crate::core::artifact_metadata::sha256_file(persisted).expect("persisted hash")
                )
            );
        });
    }
    ...
    fn external_patch_remains_visible_without_becoming_an_aggregate_apply_candidate() {
        with_isolated_home(|home| {
            ...
            let report =
                crate::core::agent_task_aggregate::AgentTaskAggregateReport::from(&[value][..]);

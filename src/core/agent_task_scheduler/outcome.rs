...
mod grouped {
    ...
    pub fn missing_required_typed_artifacts(
    ...
    pub struct InvalidRequiredTypedArtifact {
        pub task_id: String,
        pub name: String,
    ...
    pub fn invalid_required_typed_artifacts(
        outcome: &AgentTaskOutcome,
        request: &AgentTaskRequest,
    ...
    pub fn invalid_required_typed_artifact(
        outcome: &AgentTaskOutcome,
    ...
    pub fn invalid_typed_artifact_reason(
        typed_artifact: &AgentTaskTypedArtifact,
        path: Option<&str>,
        ...
        if typed_artifact.artifact.is_none() && value_is_empty(&typed_artifact.payload) {
            return Some(
                "typed artifact payload is empty and has no artifact reference".to_string(),
            );
        }
    ...
    pub fn missing_typed_artifacts_failure(outcome: &AgentTaskOutcome) -> bool {
        outcome
            .summary
            .as_deref()
    ...
    pub fn text_reports_missing_typed_artifacts(value: &str) -> bool {
        let value = value.to_ascii_lowercase();
        value.contains("missing required typed artifacts")
            || value.contains("did not produce required typed artifacts")
    }

    pub fn artifact_matches_required_artifact(name: &str, artifact: &AgentTaskArtifact) -> bool {
        [
    ...
    pub fn evidence_matches_required_artifact(name: &str, evidence: &AgentTaskEvidenceRef) -> bool {
        [
    ...
    pub fn value_matches_required_artifact(name: &str, value: &str) -> bool {
        ...
        match name.as_str() {
            "patch" => {
                value.contains("diff")
                    || value.contains("textxpatch")
                    || value.contains("textxdiff")
            }
    ...
    pub fn normalize_artifact_role_token(value: &str) -> String {
        value
            .chars()
    ...
    pub fn typed_artifact_from_artifact(
        name: &str,
    ...
    pub fn typed_artifact_from_evidence(
        name: &str,
    ...
    pub fn typed_artifact_from_outcome(outcome: &AgentTaskOutcome) -> AgentTaskTypedArtifact {
        AgentTaskTypedArtifact {
    ...
    pub fn runtime_result_is_materializable(outcome: &AgentTaskOutcome) -> bool {
    ...
    pub fn provider_run_result_is_empty_incomplete(result: &Value) -> bool {
    ...
    pub fn empty_incomplete_run_state(state: &Value) -> bool {
    ...
    #[derive(Debug, Clone, PartialEq, Eq)]
    pub struct NestedFailedExecutorStatus {
        pub path: String,
        pub key: String,
        pub value: String,
    }

    pub fn nested_failed_executor_status(
        outcome: &AgentTaskOutcome,
    ...
    pub fn find_failed_status_value(
        value: &Value,
        path: &str,
    ) -> Option<NestedFailedExecutorStatus> {
        match value {
            Value::Object(object) => {
                for (key, child) in object {
                    ...
                    if is_terminal_status_key(key) {
                        if let Some(raw) =
                            child.as_str().map(str::trim).filter(|raw| !raw.is_empty())
                        {
            ...
            Value::Array(items) => items.iter().enumerate().find_map(|(index, child)| {
                find_failed_status_value(child, &format!("{path}.{index}"))
            }),
            _ => None,
        }
    }
    ...
    pub fn is_terminal_status_key(key: &str) -> bool {
    ...
    pub fn status_value_is_failed(value: &str) -> bool {
    ...
    pub fn value_text_is_empty(value: Option<&Value>) -> bool {
        value
            .and_then(Value::as_str)
            .map(str::trim)
            .unwrap_or_default()
    ...
    pub fn has_assistant_message(result: &Value) -> bool {
    ...
    pub fn value_has_text(value: Option<&Value>) -> bool {
        match value {
    ...
    pub fn array_has_message_text(value: Option<&Value>) -> bool {
        match value {
            Some(Value::Array(items)) => items.iter().any(|item| value_has_text(Some(item))),
    ...
    pub fn array_has_assistant_role_message(value: Option<&Value>) -> bool {
    ...
    pub fn has_tool_calls(result: &Value) -> bool {
    ...
    pub fn value_has_items(value: Option<&Value>) -> bool {
        match value {
    ...
    pub fn render_template_value(raw: &str, bindings: &HashMap<String, Value>) -> Option<Value> {
    ...
    pub fn render_template_string(raw: &str, bindings: &HashMap<String, Value>) -> String {
    ...
    pub fn template_replacement(value: &Value) -> String {
        if value.is_null() {
            return String::new();
        }
    ...
    pub fn mark_generated_from_outputs(

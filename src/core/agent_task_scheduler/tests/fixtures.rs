...
mod grouped {
    ...
    #[derive(Default)]
    pub struct RetryOnceExecutor {
        pub attempts: Arc<AtomicUsize>,
    }

    #[derive(Default)]
    pub struct EmptyIncompleteThenSuccessExecutor {
        pub attempts: Arc<AtomicUsize>,
    }

    pub struct EmptyIncompleteExecutor;

    pub struct NestedFailedStatusExecutor;

    pub struct NestedTerminalStateFailedExecutor;

    pub struct NestedAgentResultFailedExecutor;

    pub struct SuccessMissingRequiredArtifactsExecutor;

    pub struct SuccessEmptyRequiredTypedArtifactExecutor {
    ...
    pub struct NestedOutputsIncompleteExecutor;

    pub struct RuntimeBundleOutcomeExecutor {
    ...
    pub struct RecordingExecutor {
        pub statuses: HashMap<String, AgentTaskOutcomeStatus>,
        pub delay: Duration,
        pub running: Arc<AtomicUsize>,
        pub max_seen: Arc<AtomicUsize>,
        pub cancel_calls: Arc<Mutex<Vec<String>>>,
    }

    pub struct OutputTemplateExecutor {
        pub observed: Arc<Mutex<Vec<AgentTaskRequest>>>,
        pub include_issue_number: bool,
    }
    ...
    impl RecordingExecutor {
        pub fn new(statuses: HashMap<String, AgentTaskOutcomeStatus>, delay: Duration) -> Self {
            Self {
                statuses,
                delay,
    ...
    pub fn plan_with_tasks(count: usize) -> AgentTaskPlan {
    ...
    pub fn plan_with_required_artifacts(names: &[&str]) -> AgentTaskPlan {
    ...
    pub fn request(task_id: &str) -> AgentTaskRequest {
        AgentTaskRequest {
    ...
    pub fn outcome(task_id: String, status: AgentTaskOutcomeStatus) -> AgentTaskOutcome {
        AgentTaskOutcome {
            schema: AGENT_TASK_OUTCOME_SCHEMA.to_string(),
            task_id,
            status,
    ...
    pub fn empty_incomplete_outcome(task_id: String) -> AgentTaskOutcome {
    ...
    pub fn nested_outputs_incomplete_outcome(task_id: String) -> AgentTaskOutcome {
        let mut outcome = outcome(task_id, AgentTaskOutcomeStatus::Succeeded);

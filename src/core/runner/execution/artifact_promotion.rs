...
use crate::core::performance_hotspots::PERFORMANCE_HOTSPOTS_SUMMARY_SCHEMA;
...
) -> crate::core::Result<Vec<ArtifactRecord>> {
...
fn remote_runner_exec_artifact_type(
    ...
    path: &Path,
) -> crate::core::Result<RunnerExecArtifactSourceType> {
    let path_string = path.display().to_string();
...
fn copy_dir_all(from: &Path, to: &Path) -> crate::core::Result<()> {
...
fn validate_runner_exec_artifact_path(
    ...
    match crate::core::paths::authorize_remote_artifact_path(
        path,
        &roots,
        crate::core::paths::RemotePathRootContainment::NativePath,
    ) {
        Ok(()) => Ok(()),
        Err(crate::core::paths::RemotePathAuthorizationError::NotAbsolute) => {
            Err(Error::validation_invalid_argument(
                "path",
        ...
        Err(crate::core::paths::RemotePathAuthorizationError::ContainsParentDir) => {
            Err(Error::validation_invalid_argument(
                "path",
        ...
        Err(crate::core::paths::RemotePathAuthorizationError::OutsideAllowedRoots) => {
            Err(Error::validation_invalid_argument(
                "path",
...
fn runner_exec_attach_download_path(runner_id: &str, path: &Path) -> crate::core::Result<PathBuf> {
    let file_name = path
        .file_name()

...
pub(super) struct RunnerGitDependencyMaterializationOutput {
    pub local_path: String,
    pub remote_path: String,
...
#[derive(Debug, Clone)]
pub(super) struct RunnerGitDependencyMaterializationOptions {
    pub local_path: String,
    pub remote_path: String,
...
pub(super) fn materialize_git_dependency(
    runner: &Runner,
...
pub(super) fn materialize_git_dependency_command(
    remote_path: &str,
    remote_url: &str,
    head: &str,
...
fn run_shell_command(command: &str, action: &str) -> Result<()> {
    run_shell_command_with_stdout(command, action).map(|_| ())
}

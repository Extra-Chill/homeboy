...
mod grouped {
    ...
    pub const CONTROLLER_EVIDENCE_INDEX_SCHEMA: &str =
        "homeboy/agent-task-loop-controller-evidence-index/v1";

    #[derive(Debug, Clone, Serialize)]
    pub struct ControllerEvidenceIndex {
        schema: &'static str,
        run_id: String,
    ...
    #[derive(Debug, Clone, Serialize)]
    pub struct ControllerEvidenceIndexEntry {
        task_id: String,
        #[serde(default, skip_serializing_if = "Vec::is_empty")]
    ...
    pub struct RequiredWorkflowArtifact {
        artifact_id: String,
        kind: String,
    ...
    pub fn validate_required_action_artifacts(
    ...
    pub fn missing_required_artifacts_for_execution(
        request: &Value,
        execution: &Value,
        scope: Option<String>,
    ) -> Vec<RequiredWorkflowArtifact> {
    ...
    pub fn required_workflow_artifacts(request: &Value) -> Vec<RequiredWorkflowArtifact> {
        let mut artifacts = Vec::new();
        collect_required_artifacts_from_plan(
            request.get("plan").unwrap_or(request),
            &mut artifacts,
        );
    ...
    pub fn collect_required_artifacts_from_client_context(
        value: &Value,
    ...
    pub fn collect_required_artifacts_from_plan(
        value: &Value,
        artifacts: &mut Vec<RequiredWorkflowArtifact>,
    ) {
    ...
    pub fn collect_required_artifacts_from_declarations(
        value: &Value,
        artifacts: &mut Vec<RequiredWorkflowArtifact>,
    ) {
    ...
    pub fn request_with_required_workflow_artifacts(
    ...
    pub fn hydrate_consumed_artifacts(
        record: &AgentTaskLoopControllerRecord,
        request: &Value,
    ) -> Value {
    ...
    pub fn find_controller_artifact(
        record: &AgentTaskLoopControllerRecord,
        artifact_id: &str,
    ) -> Option<Value> {
    ...
    pub fn merge_request_input_artifacts(
        request: &mut Value,
    ...
    pub fn merge_runtime_execution_input_artifacts(
        request: &mut Value,
        artifacts: &serde_json::Map<String, Value>,
    ) {
    ...
    pub fn insert_artifact_aliases(
    ...
    pub fn runtime_bundle_evidence(request: &Value, execution: &Value) -> Option<Value> {
    ...
    pub fn runtime_bundle_configurations(value: &Value) -> Vec<Value> {
        let mut tasks = Vec::new();
        collect_runtime_bundle_configurations(value, &mut tasks, 0);
        tasks
    }

    pub fn collect_runtime_bundle_configurations(
        value: &Value,
        tasks: &mut Vec<Value>,
        depth: usize,
    ) {
    ...
    pub fn push_runtime_bundle_configuration(runtime_task: &Value, tasks: &mut Vec<Value>) {
    ...
    pub fn runtime_budget_fields(value: &Value) -> serde_json::Map<String, Value> {
        let mut fields = serde_json::Map::new();
        collect_runtime_budget_fields(value, &mut fields, 0);
        fields
    }

    pub fn collect_runtime_budget_fields(
        value: &Value,
    ...
    pub fn is_runtime_budget_key(key: &str) -> bool {
        key == "time_budget_ms"
            || key == "step_budget"
            || key == "wait_budget_ms"
            || key == "drain_budget_ms"
    ...
    pub fn runtime_bundle_observations(value: &Value) -> Vec<Value> {
        let mut observations = Vec::new();
        collect_runtime_bundle_observations(value, &mut observations, 0);
        observations
    }

    pub fn collect_runtime_bundle_observations(
        value: &Value,
    ...
    pub fn is_runtime_observation_key(key: &str) -> bool {
        matches!(
            key,
            "wait_result"
                | "job_status"
    ...
    pub fn matching_completed_workflow_artifacts(
        record: &AgentTaskLoopControllerRecord,
        request: &Value,
    ) -> Vec<Value> {
    ...
    pub fn merge_workflow_artifacts(
    ...
    pub fn required_artifact_diagnostics(
    ...
    pub fn record_controller_spawn(
    ...
    pub fn record_controller_aggregate_evidence(
        record: &mut AgentTaskLoopControllerRecord,
        entity_id: Option<&str>,
        run_id: &str,
    ...
    pub fn record_controller_result_evidence(
        record: &mut AgentTaskLoopControllerRecord,
        entity_id: Option<&str>,
        run_id: &str,
        result: &Value,
    ...
    pub fn record_controller_evidence_index(
        ...
        if let Some(entity_id) = entity_id {
            if let Some(entity) = record.entities.get_mut(entity_id) {
                ...
                entity.metadata =
                    merge_entity_evidence_output(entity.metadata.clone(), index_value);
            }
        }
    ...
    pub fn merge_controller_evidence_output(outputs: Value, index: Value) -> Value {
        let mut object = outputs.as_object().cloned().unwrap_or_default();
        object.insert("evidence_index".to_string(), index);
        Value::Object(object)
    }

    pub fn merge_entity_evidence_output(metadata: Value, index: Value) -> Value {
    ...
    pub fn parse_array<T>(value: &Value) -> Result<Vec<T>>
    where
        T: DeserializeOwned,
    {
        if value.is_null() {
    ...
    pub fn controller_action_runtime_evidence(
        execution: &Value,
    ...
    pub fn set_first_string(target: &mut Option<String>, value: Option<&str>) {
        if target.is_none() {
    ...
    pub fn collect_ref_group(value: Option<&Value>, target: &mut Vec<Value>) {
        match value {
    ...
    pub fn collect_ref_field(value: Option<&Value>, target: &mut Vec<Value>) {
        if let Some(value) = value.filter(|value| !value.is_null()) {
            push_ref_once(target, value.clone());
        }
    }

    pub fn push_ref_once(target: &mut Vec<Value>, value: Value) {

...
fn merge_runtime_task_input_artifacts(request: &mut Value, artifacts: &serde_json::Map<String, Value>) {
    let Some(runtime_input) = request
        .get_mut("inputs")
...
mod tests {
    ...
    fn consumed_artifact_ids_include_artifact_dependencies() {
        ...
        assert_eq!(
            request["inputs"]["runtime_task"]["input"]["input"]["artifacts"]["concept_packet"]["payload"]["title"],
            "Kiln Shelf Supply"
        );
    }
}

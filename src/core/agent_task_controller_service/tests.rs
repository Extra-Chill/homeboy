...
mod resume_tests {
    ...
    fn resume_failed_action_result_includes_top_level_failure_summary() {
        with_isolated_home(|_| {
            ...
            assert_eq!(
                failed["execution"]["result"]["aggregate"]["outcomes"][0]["diagnostics"][0]
                    ["message"],
                failed["failure_summary"]["diagnostic"]
            );
        });
    ...
    fn resume_with_options_stops_at_max_actions_with_pending_work_remaining() {
        with_isolated_home(|_| {
            ...
            let loaded = controller::load_controller("loop-service-bounded-resume")
                .expect("controller loaded");
            assert_eq!(
                loaded.next_actions[0].status,
    ...
    fn resume_with_options_stops_after_terminal_state() {
        with_isolated_home(|_| {
            ...
            let loaded = controller::load_controller("loop-service-terminal-resume")
                .expect("controller loaded");
    ...
    fn fan_out_indexes_each_child_task_evidence_on_its_entity() {
        with_isolated_home(|_| {
            ...
            for entity_id in [first, second] {
                ...
                assert_eq!(entity.artifact_refs.len(), 3);
                assert_eq!(
                    entity.metadata["outputs"]["evidence_indexes"][0]["entries"][0]
                        ["typed_artifacts"][0]["artifact_schema"],
    ...
    fn run_command_workflow_executes_deterministic_artifact_action() {
        with_isolated_home(|home| {
            ...
            let persisted: Value = serde_json::from_str(
                &std::fs::read_to_string(&persisted_artifact)
                    .expect("persisted controller artifact"),
            )
...
mod dispatch_execute_tests {
    ...
    fn execute_controller_action_fails_closed_for_unimplemented_runner_target() {
        with_isolated_home(|_| {
            let mut record =
                AgentTaskLoopControllerRecord::new("loop-runner-target", "dispatch", "v1");
            let action = record.record_action(
                AgentTaskLoopPolicyAction::SpawnTask {
    ...
    fn execute_controller_dispatch_injects_action_scoped_identity() {
        with_isolated_home(|_| {
            let mut record =
                AgentTaskLoopControllerRecord::new("loop-dispatch-identity", "init", "v1");
            let action = record.record_action(
                AgentTaskLoopPolicyAction::SpawnTask {
            ...
            assert!(run_id
                .starts_with("controller-loop-dispatch-identity-action-1-workflow_store-idea"));
            assert!(task_id.starts_with(
                "controller-task-loop-dispatch-identity-action-1-workflow_store-idea"
            ));

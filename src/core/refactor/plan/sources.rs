...
/// Homeboy-owned CI artifact path prefixes.
///
...
/// the only uncommitted changes are under these prefixes, write-mode
/// refactor/lint-fix is safe to proceed (#4684).
const HOMEBOY_OWNED_CI_ARTIFACT_PREFIXES: &[&str] =
    &["homeboy-ci-results/", "homeboy-observations/"];

/// Whether a working-tree path is a Homeboy-owned CI artifact.
///
/// Matches [`HOMEBOY_OWNED_CI_ARTIFACT_PREFIXES`] after stripping a leading
/// `./` and normalizing backslashes. Both the directory entry itself
...
/// individual staged/unstaged file paths alike.
fn is_homeboy_owned_ci_artifact(path: &str) -> bool {
    let normalized = path.trim_start_matches("./").replace('\\', "/");
    HOMEBOY_OWNED_CI_ARTIFACT_PREFIXES
        .iter()
...
fn ensure_clean_enough_for_write(
    ...
    if dirty_changes_are_only_homeboy_artifacts(changes) {
        crate::log_status!(
            "refactor",
            "Working tree is dirty only with Homeboy-owned CI artifacts; \
             allowing write-mode refactor (homeboy#4684)"
        );
        return Ok(());
    }
...
mod tests {
    ...
    fn ensure_clean_enough_for_write_allows_artifact_only_dirty_tree() {
        let root = PathBuf::from("/tmp/homeboy-artifact-only-guard");
        let request = lint_refactor_request(
            test_component(&root),
            root,
            Vec::new(),

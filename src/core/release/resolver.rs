...
mod tests {
    ...
    #[test]
    fn test_resolve_extensions_if_let_some_configured_component_extensions_as_ref() {

        let result = resolve_extensions();
        assert!(!result.is_empty(), "expected non-empty collection for: if let Some(configured) = component.extensions.as_ref() {{");
    }

    #[test]
    fn test_resolve_extensions_default_path() {
    ...
    #[test]
    fn test_resolve_extensions_ok_extensions() {

        let result = resolve_extensions();
        assert!(!result.is_empty(), "expected non-empty collection for: Ok(extensions)");
    }

    #[test]
    fn test_resolve_extensions_has_expected_effects() {
        // Expected effects: mutation

        let _ = resolve_extensions();
    }

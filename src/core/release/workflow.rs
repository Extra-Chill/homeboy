...
mod tests {
    ...
    fn test_run_batch_match_run_command_input() {
        ...
        assert!(result.is_ok(), "expected Ok for: match run_command(input)");
    }

    #[test]
    fn test_run_batch_result_some_result() {
        let result = run_batch();
        assert!(result.is_ok(), "expected Ok for: result: Some(result),");
    }
    ...
    fn test_run_batch_error_some_e_to_string() {
        ...
        assert!(
            result.is_ok(),
            "expected Ok for: error: Some(e.to_string()),"
        );
    }

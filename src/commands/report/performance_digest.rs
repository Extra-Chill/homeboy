...
fn envelope_data(value: &Value) -> Map<String, Value> {
...
fn resource_summary_digest(value: &Value) -> Option<ResourceSummaryDigest> {
...
fn load_digest(value: &Value) -> Option<LoadDigest> {
    let obj = value.as_object()?;
    Some(LoadDigest {
...
fn child_resource_digest(value: Value) -> Option<ChildResourceDigest> {
    let obj = value.as_object()?;
...
fn collect_budget_findings(data: &Map<String, Value>) -> Vec<BudgetFindingDigest> {
...
fn collect_baseline_health(
    ...
    for scenario in array_value(&results, "scenarios") {
        ...
        let scenario_id = string_value(scenario_obj, "id").unwrap_or_else(|| "unknown".to_string());
...
fn collect_metadata_baseline_health(
    metadata: &Value,
    min_samples: u64,
    max_cv_pct: f64,
) -> Vec<BaselineHealthDiagnostic> {
...
fn collect_runs_summary_diagnostics(
...
fn host_pressure_digest(obj: &Map<String, Value>) -> Option<HostPressureDigest> {
...
fn render_resource_summary(out: &mut String, summary: Option<&ResourceSummaryDigest>) {
...
fn render_budget_findings(out: &mut String, findings: &[BudgetFindingDigest]) {
...
fn render_baseline_health(out: &mut String, diagnostics: &[BaselineHealthDiagnostic]) {
...
fn render_host_pressure(out: &mut String, host_pressure: Option<&HostPressureDigest>) {
...
fn render_lab_offload(out: &mut String, lab_offload: &BTreeMap<String, String>) {
...
fn render_gaps(out: &mut String, gaps: &[String]) {
...
fn find_object_recursive<'a>(value: &'a Value, key: &str) -> Option<&'a Map<String, Value>> {
    match value {
        Value::Object(map) => {
...
fn find_number_recursive(value: &Value, key: &str) -> Option<f64> {
    match value {
...
fn scalar_object(map: &Map<String, Value>) -> BTreeMap<String, String> {
    map.iter()
...
fn dedupe_diagnostics(diagnostics: Vec<BaselineHealthDiagnostic>) -> Vec<BaselineHealthDiagnostic> {
...
fn array_value(map: &Map<String, Value>, key: &str) -> Vec<Value> {
    map.get(key)
...
fn format_load(load: &LoadDigest) -> String {
    format!(
        "1m={} 5m={} 15m={}",
        load.one
            .map(format_number)

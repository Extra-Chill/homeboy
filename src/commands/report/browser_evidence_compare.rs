...
#[derive(Debug, Clone)]
struct EvidenceSet {
    samples: Vec<BrowserEvidenceSample>,
    notes: Vec<String>,
}

#[derive(Debug, Clone, Default)]
struct BrowserEvidenceSample {
...
pub fn browser_evidence_compare_from_args(
    ...
    let baseline = read_evidence_set(&baseline_dir, args.include_local_paths)?;
    let candidate = read_evidence_set(&candidate_dir, args.include_local_paths)?;
    ...
    let variants = compare_variants(&baseline.samples, &candidate.samples);
...
fn read_evidence_set(root: &Path, include_local_paths: bool) -> homeboy::core::Result<EvidenceSet> {
    let mut notes = Vec::new();
...
fn collect_metric_object(value: Option<&Value>, out: &mut BTreeMap<String, f64>, names: &[&str]) {

...
/// 4. Rewrite all `use homeboy_core::old::path::module` → `use homeboy_core::new::path::module` across the codebase
pub fn move_file(from: &str, to: &str, root: &Path, write: bool) -> Result<MoveFileResult> {
    ...
    if let Some(ref ext) = ext {
        if source_module != dest_module {
            ...
            // Also include the module name itself for `use homeboy_core::path::module;` imports
    ...
    if write {
        ...
        std::fs::rename(&source_abs, &dest_abs).map_err(|e| {
            homeboy_core::Error::internal_io(e.to_string(), Some(format!("move {} → {}", from, to)))
        })?;

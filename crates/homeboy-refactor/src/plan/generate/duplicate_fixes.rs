...
///
/// For Rust: `use homeboy_core::module::path::function_name;`
...
pub(crate) fn generate_duplicate_function_fixes(
    ...
    for group in &result.duplicate_groups {
        ...
        let use_extract_shared = matches!(language, Language::Php)
            && !homeboy_code_audit::is_test_path(&group.canonical_file);
...
fn generate_simple_duplicate_fixes(
    ...
    for remove_file in &group.remove_from {
        ...
        if homeboy_code_audit::walker::is_test_path(remove_file)
            || homeboy_code_audit::walker::is_test_path(&group.canonical_file)
        {
            skip_file(
                skipped,
                remove_file.clone(),
...
mod tests {
    ...
    fn test_generate_duplicate_import_rust() {
        ...
        assert_eq!(import, "use crate::engine::symbol_graph::parse_imports;");
    }

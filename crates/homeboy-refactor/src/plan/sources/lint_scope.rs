...
pub(super) fn capture_release_owned_files(
    component: &Component,
    root: &Path,
) -> homeboy_core::Result<Vec<ReleaseOwnedFileSnapshot>> {
...
) -> homeboy_core::Result<LintFixScopeOutcome> {
...
fn lint_autofix_diff(root: &Path, changed_files: &[String]) -> homeboy_core::Result<String> {

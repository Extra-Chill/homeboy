...
pub struct RefactorSourceRun {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub guard_block: Option<crate::auto::guard::GuardBlock>,
}
...
pub fn collect_refactor_sources(
    ...
    if request.write {
        let guard_config = crate::auto::guard::GuardConfig::from_env();
        match crate::auto::guard::check_guards(&root_str, &guard_config) {
            crate::auto::guard::GuardResult::Proceed => {}
            crate::auto::guard::GuardResult::Blocked(block) => {
                eprintln!(
    ...
    if applied {
        ...
        require_successful_format(
            homeboy_core::engine::format_write::format_after_write(&request.root, &abs_changed)?,
    ...
    match git::run_git_output(root, &["status", "--short"], "git status --short") {
        Ok(output) if output.status.success() => {
            ...
            if status.trim().is_empty() {
                homeboy_core::log_status!("refactor", "git status --short: <clean>");
            } else {
                homeboy_core::log_status!("refactor", "git status --short:\n{}", status.trim_end());
            }
        }
        Ok(output) => {
...
fn clean_homeboy_owned_ci_artifacts(root: &Path) -> homeboy_core::Result<bool> {
...
fn ensure_clean_enough_for_write(
    request: &RefactorSourceRequest,
    changes: Option<&homeboy_core::git::UncommittedChanges>,
) -> homeboy_core::Result<()> {
...
pub(crate) fn normalize_sources(sources: &[String]) -> homeboy_core::Result<Vec<String>> {
...
mod tests {
    ...
    fn formatter_failure_is_a_write_failure() {
        let result = require_successful_format(
            homeboy_core::engine::format_write::FormatResult {
                success: false,
    ...
    use crate::auto::{Fix, FixResult, Insertion, InsertionKind, NewFile};

use crate::auto::{FixPolicy, FixResult, Insertion, NewFile, PolicySummary};
...
fn should_auto_apply(finding: &AuditFinding, manual_only: bool) -> bool {
    !manual_only
        && homeboy_code_audit::findings::finding_confidence(finding).allows_automated_refactor()
}
...
fn blocked_reason(finding: &AuditFinding, manual_only: bool) -> String {
    ...
    } else if !homeboy_code_audit::findings::finding_confidence(finding).allows_automated_refactor()
    {
        format!(
            "Blocked: {:?} confidence finding requires human review before automated writes",
            homeboy_code_audit::findings::finding_confidence(finding)
        )
    } else {
...
mod tests {
    use super::*;
    use crate::auto::{Fix, FixResult, InsertionKind};

...
use homeboy_extension_contract::autofix_config::AutofixVerifyConfig;
use std::path::Path;
...
pub(crate) fn apply_fixes_via_edit_ops(
    ...
    for (file, nf_index) in &new_file_index {
        ...
        if error_files.contains(file.as_str()) {
            ...
            nf.written = true;
            homeboy_core::log_status!("fix", "Created {}", nf.file);
    ...
    if !formatted_files.is_empty() {
        let _ = homeboy_core::engine::format_write::format_after_write(root, &formatted_files);
    }
...
mod tests {
    use super::*;
    use crate::auto::{Insertion, InsertionKind};

...
fn stale_persisted_lease_refuses_an_unverified_live_daemon_with_recovery_guidance() {
    ...
    let err = remote_daemon_connect_action_for_runner(
        Some(&session),
        &status,
        "homeboy test+configured",
        "<runner-id>",
...
fn connect_reports_local_runner_as_unsupported() {
    test_support::with_isolated_home(|_| {
        crate::create(r#"{"id":"lab-local","kind":"local"}"#, false).expect("create runner");
...
fn sessionless_active_daemon_prescribes_matching_pinned_controller_on_identity_mismatch() {
    ...
    let error = remote_daemon_connect_action_for_runner(
        None,
        &status,

...
use homeboy_core::change_artifact::{
    ChangeApplyResult, ChangeApplyStatus, ChangeArtifact, ChangeDelta, ChangePatch,
    UNIFIED_DIFF_PATCH_FORMAT,
};
use homeboy_core::engine::command::{wait_with_bounded_output, DEFAULT_CAPTURE_LIMIT_BYTES};
    ...
    match serde_json::from_str(&contents) {
        ...
        Err(contract_error) => {
            ...
            Ok(LabPatchApplyInput {
                artifact: ChangeArtifact {
                    schema: homeboy_core::change_artifact::CHANGE_ARTIFACT_SCHEMA.to_string(),
...
mod tests {
    ...
    fn output_serializes_flat_apply_result_with_artifact_summary() {
        ...
        fs::write(
            &input,
            serde_json::json!({
                "schema": homeboy_core::change_artifact::CHANGE_ARTIFACT_SCHEMA,
                "source_snapshot": snapshot,
                "provenance": {
        ...
        assert_eq!(
            json["schema"],
            homeboy_core::change_artifact::CHANGE_APPLY_RESULT_SCHEMA
        );
        assert_eq!(json["apply_status"], "applied");
        assert_eq!(json["modified_files"], serde_json::json!(["file.txt"]));
        assert_eq!(
            json["artifact"]["schema"],
            homeboy_core::change_artifact::CHANGE_ARTIFACT_SCHEMA
        );
        assert_eq!(

...
use homeboy_core::api_jobs::{JobEventKind, JobStore, RemoteRunnerJobResult};
...
pub(super) fn write_reverse_controller_session(broker_url: &str) {
    let path = homeboy_core::paths::runner_session_file("lab").expect("session path");
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).expect("create session dir");
    }
    let session = crate::RunnerSession {

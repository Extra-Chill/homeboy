...
pub(super) fn portable_lab_command(label: &'static str) -> LabOffloadCommand {
    LabOffloadCommand {
        command: homeboy_core::lab_contract::LabCommandContract::portable(label, None, true, &[]),
...
pub(super) fn local_only_lab_command(reason: &'static str) -> LabOffloadCommand {
    LabOffloadCommand {
        command: homeboy_core::lab_contract::LabCommandContract::local_only("rig up", reason),
        required_extensions: Vec::new(),
        required_capabilities: Vec::new(),
        workload: None,
    }
...
pub(super) fn truncated_runner_exec_output() -> RunnerExecOutput {
    RunnerExecOutput {
        ...
        metrics: None,
        capture: Some(homeboy_core::engine::command::CommandCaptureMetadata {
            stdout: homeboy_core::engine::command::CaptureMetadata {
                bytes_seen: 5 * 1024 * 1024,
                bytes_retained: 4 * 1024 * 1024,
                byte_limit: 4 * 1024 * 1024,
                bytes_truncated: 1024 * 1024,
                truncated: true,
                sha256: "sha256:test".to_string(),
            },
            stderr: homeboy_core::engine::command::CaptureMetadata {
                bytes_seen: 0,
                bytes_retained: 0,
                byte_limit: 4 * 1024 * 1024,
                bytes_truncated: 0,

...
fn normalize_remote_path(path: &str) -> Result<String> {
    ...
    Ok(homeboy_core::paths::normalize_remote_root(path))
}
...
fn allowed_runner_workspace_roots(runner: &Runner) -> Result<Vec<String>> {
    ...
    if let Some(root) = runner.workspace_root.as_deref() {
        roots.push(homeboy_core::paths::normalize_remote_root(root));
    }
    roots.extend(
        runner
            .policy
            .workspace_roots
            .iter()
            .map(|root| homeboy_core::paths::normalize_remote_root(root)),
    );
    roots.sort();
    roots.dedup();
...
fn path_is_within_root(path: &str, root: &str) -> bool {
    homeboy_core::paths::remote_path_is_within_root(path, root)
}

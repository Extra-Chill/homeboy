...
macro_rules! builtin_json_command_descriptors {
    ($consumer:ident) => {
        $consumer! {
            ...
            (Topology, $crate::commands::topology::run, command_spec_with_output_notes("topology", CommandJsonFamily::Workspace, "read-only declared resource identities, edges, roots, and unresolved-reference diagnostics")),

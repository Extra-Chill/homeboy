...
pub fn run(args: RunnerArgs) -> CmdResult<RunnerCommandOutput> {
    match args.command {
        ...
        RunnerCommand::Connect {
            ...
            &id,
            registry::RunnerConnectInput {
                reverse,
                runner_id: reverse_runner,
                broker_url,
                adopt_orphan_lease,
                adopt_live_lease,
                expected_live_pid,
        ...
        RunnerCommand::RefreshHomeboy {
            ...
            map_refresh_homeboy_with_admission(&runner_id, result, Some(admission))
        }
        ...
        RunnerCommand::Exec {
            ...
            raw,
            command,
        } => map_execution(execute(RunnerExecInput {
            runner_id: id,
            command,
            cwd,
            sync_workspace,
        ...
        RunnerCommand::RecipeProviders => Ok((
            RunnerCommandOutput::RecipeRunProviders(Box::new(RecipeRunProvidersOutput {
                ...
                command: "list",
                providers: super::recipe_run::recipe_run_provider_inventory()?,
            })),
            0,
        )),
...
pub(super) fn run_exec_command(input: RunnerExecInput, json: bool) -> CommandRun {
    let raw = input.raw;
    match execute(input) {
        Ok((output, exit_code)) if json => {
            let (stdout_result, _) =
                crate::commands::utils::response::map_cmd_result_to_json(Ok((
                    RunnerCommandOutput::Execution(Box::new(runner_exec_command_output(output))),
                    exit_code,
                )));
            CommandRun::from_stdout_result(stdout_result, exit_code)
        }
        Ok((output, exit_code)) if raw => raw_exec_command_run(output, exit_code),
...
struct RefreshProgress {
    admission: Option<RefreshRunAdmission>,
    stop: Arc<AtomicBool>,
...
impl RefreshProgress {
    fn admit(
        ...
        let run = store.start_run(
            NewRunRecord::builder("runner_refresh_homeboy")
                ...
                .metadata(serde_json::json!({
                    ...
                    "progress": { "phase": phase, "state": "running", "heartbeat": true }
                }))
                .build(),
        )?;
        let run_id = run.id;
        eprintln!(
            "HOMEBOY_REFRESH_PROGRESS {{\"run_id\":\"{}\",\"phase\":\"{}\",\"state\":\"running\"}}",
            run_id, phase
        );
        ...
        let heartbeat_phase = phase.to_string();
        ...
        let worker = thread::spawn(move || {
            while !worker_stop.load(Ordering::Acquire) {
                ...
                eprintln!(
                    "HOMEBOY_REFRESH_PROGRESS {{\"run_id\":\"{}\",\"phase\":\"{}\",\"state\":\"running\",\"heartbeat\":true}}",
                    heartbeat_run_id, heartbeat_phase
                );
            }
        });

        Ok(Self {
            admission: Some(RefreshRunAdmission { store, run_id }),
            stop,
            worker: Some(worker),
        })
    }
    ...
    fn finish(mut self) -> RefreshRunAdmission {
        ...
        self.admission.take().expect("refresh admission")
    }
}
...
fn map_refresh_homeboy_with_admission(
    ...
    match result {
        Err(mut error) => {
            let artifacts = persist_refresh_error_artifacts(runner_id, &error, admission)?;
...
fn persist_refresh_artifacts(
    ...
    let (store, run_id) = match admission {
        Some(admission) => (admission.store, admission.run_id),
        None => {
            let store = ObservationStore::open_initialized()?;
            let run = store.start_run(
                NewRunRecord::builder("runner_refresh_homeboy")
...
mod tests {
    ...
    fn refresh_failure_maps_parent_run_job_evidence_and_recovery_actions() {
        homeboy::test_support::with_isolated_home(|_| {
            ...
            let (mapped, exit_code) =
                map_refresh_homeboy_with_admission("lab", Ok((output, 2)), None)
    ...
    fn refresh_success_persists_the_complete_build_transcript() {
        homeboy::test_support::with_isolated_home(|_| {
            ...
            let (mapped, exit_code) =
                map_refresh_homeboy_with_admission("lab", Ok((output, 0)), None)
                    .expect("mapped output");
            assert_eq!(exit_code, 0);
            let value = serde_json::to_value(mapped).expect("JSON envelope");
            let run_id = value["artifacts"]["run_id"]
    ...
    fn refresh_error_persists_a_durable_error_artifact() {
        homeboy::test_support::with_isolated_home(|_| {
            ...
            let error = map_refresh_homeboy_with_admission("lab", Err(error), None)

...
fn rig_trace_workload_port_range_sets_invocation_env() {
    with_isolated_home(|home| {
        ...
        let (output, exit_code) = run(trace_args_for_rig(
            "studio-rig",
            "studio",
            "studio-app-create-site",
        ))
...
fn trace_run_preserves_nested_child_artifact_directories() {
    with_isolated_home(|home| {
        ...
        let (_output, exit_code) = run(trace_args_for_rig(
            "studio-rig",
            "studio",
            "studio-app-create-site",
        ))
...
fn trace_run_resolves_named_variants_and_reports_unknown_names() {
    with_isolated_home(|home| {
        ...
        let (output, exit_code) = run(valid_args.clone()).expect("variant trace should run");
        ...
        let err = match run(invalid_args) {

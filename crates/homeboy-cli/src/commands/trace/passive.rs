...
use super::PassiveTraceArgs;
use crate::commands::utils::response::actionable_metadata_value_for_run_ref;
use crate::commands::CmdResult;

pub(super) const DEFAULT_DURATION: &str = "30s";
pub(super) const DEFAULT_PROCESS_WATCH_INTERVAL: &str = "1s";

pub(super) fn run(args: PassiveTraceArgs) -> CmdResult<TraceCommandOutput> {
    ...
    let finished = observation.store().finish_run(
        observation.run_id(), status,
        Some(serde_json::json!({
            "duration_ms": duration_millis(args.duration), "event_count": results.timeline.len(),
            "trace_results_artifact_id": artifact.id, "trace_results_path": artifact.path, "failure": failure,
        })),
    )?;
    run_dir.cleanup();
    Ok((
        TraceCommandOutput::Passive(TracePassiveOutput {
            command: "observe",
...
fn trace_probes(args: &PassiveTraceArgs) -> homeboy::core::Result<Vec<TraceProbeConfig>> {
    let mut probes = args
        .probes
        .iter()
        .map(|raw| {
    ...
    if probes.is_empty() {
        return Err(Error::validation_invalid_argument(
            "probe",
            "trace observe requires at least one --tail-log, --watch-process, or --probe",
            None,
            None,
        ));
    }
...
fn observe_command(args: &PassiveTraceArgs) -> String {
    let mut parts = vec![
        "homeboy".to_string(),
        "trace".to_string(),
        "observe".to_string(),
...
mod tests {
    ...
    fn legacy_flags_translate_to_typed_trace_probes() {
        let args = PassiveTraceArgs {
            comp: crate::commands::utils::args::PositionalComponentArgs {

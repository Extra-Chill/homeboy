...
fn reconcile_fanout_pr_states(batch_id: &str, mutate: bool) -> Result<BTreeMap<String, String>> {
    homeboy::agents::agent_task_fanout_service::reconcile_fanout_pr_states(batch_id, mutate)
}

#[cfg(test)]
fn native_worktree_disposition(
    status: &str,
    exit_code: i32,
) -> Option<homeboy::core::worktree_provider::WorktreeTerminalDisposition> {
    homeboy::agents::agent_task_fanout_service::native_worktree_disposition(status, exit_code)
}

#[cfg(test)]
use homeboy::agents::agent_task_fanout_service::FanoutPrObservation;
...
fn portfolio_observation(
    ...
    homeboy::agents::agent_task_fanout_service::portfolio_observation(
        child_id,
        run_id,
        reconcile,
        declared_tracker,
...
fn github_observation(
    path: &str,
    record: Option<&homeboy::agents::agent_task_lifecycle::AgentTaskRunRecord>,
    finalization: Option<&Value>,
    declared_tracker: Option<&str>,
) -> Result<(
    homeboy::agents::agent_tasks::fanout_supervisor::AgentTaskFanoutTrackerState,
    homeboy::agents::agent_tasks::fanout_supervisor::AgentTaskFanoutPrState,
    Option<String>,
    Vec<homeboy::agents::agent_tasks::fanout_supervisor::AgentTaskFanoutReviewFinding>,
)> {
    homeboy::agents::agent_task_fanout_service::github_observation(
        path,
        record,
        finalization,
        declared_tracker,
...
fn declared_tracker_ref(metadata: &Value) -> Option<&str> {
    homeboy::agents::agent_task_fanout_service::declared_tracker_ref(metadata)
}

#[cfg(test)]
fn force_with_lease_push(path: &str, head: &str) -> Result<Value> {
    homeboy::agents::agent_task_fanout_service::force_with_lease_push(path, head)
}
...
fn batch_resume_result(
    ...
    (
        serde_json::json!({
            ...
            "cooks": serde_json::to_value(report.cooks).unwrap_or(Value::Null),
            "portfolio": portfolio,
            "commands": {
...
impl DryRunPlanner {
    ...
    fn run_bounded<T: Send + 'static>(
        ...
        self.begin(phase);
        let Some(mut remaining) = self.deadline.checked_duration_since(Instant::now()) else {
...
fn run_batch_cook_fanout_plan_with_attempt_dispatcher_claim(
    ...
    let outcome = with_current_cook_deadline(deadline, || {
        ...
        heartbeat.finish()?;
        let result = result?;
        finalize_native_worktrees(&plan, &result.value)?;
...
fn finalize_native_worktrees(
    ...
    for cell in &report.cooks {
        ...
        let Some(disposition) =
            homeboy::agents::agent_task_fanout_service::native_worktree_disposition(
                cell.status.as_str(),
                cell.exit_code,
            )
        else {
        ...
        if homeboy::agents::agent_task_fanout_service::portfolio_vetoes_success_cleanup(
            &plan.fanout_id,
            &cook.run_id(),
            disposition,
        )? {
        ...
        // children as failed.
        let result = homeboy::agents::agent_task_fanout_service::finalize_fanout_native_worktree(
            &plan.fanout_id,
            &cook.run_id(),
            &cook.to_worktree,
            &cook.run_id(),
        ...
        if cook.adopted_worktree {
            let workspace =
                ...
                .ok_or_else(|| {
                    Error::validation_invalid_argument(
                        "worktree",
                        "explicit worktree is not a native Homeboy worktree",
...
fn active_registered_worktree_path(handle: &str) -> Option<String> {
    homeboy::core::worktree_provider::resolve_native_worktree_mutation_target(handle)
        .ok()
        .flatten()
...
fn build_cook_batch_plan_with_profiles(
    ...
    let worktree_repo = repository.clone();
...
mod tests {
    ...
    fn persist_native_finalization_test_batch(plan: &BatchCookFanoutPlan) {
        let children = plan
            .cooks
            .iter()
            .map(|cook| batch::FanoutRunBatchChild {
        ...
        .expect("persist native finalization test batch");
    }

    #[test]
    fn green_and_awaiting_acceptance_are_not_native_terminal() {
        assert_eq!(native_worktree_disposition("green_no_finalize", 0), None);
        assert_eq!(native_worktree_disposition("awaiting_acceptance", 1), None);
        assert_eq!(native_worktree_disposition("review_form_timeout", 1), None);
    }
    ...
    fn dispatcher_fanout_finalizes_native_failure_as_preserved() {
        with_isolated_home(|home| {
            ...
            persist_native_finalization_test_batch(&plan);

            finalize_native_worktrees(&plan, &report).expect("native terminal finalization");
...
"#,
    ...
    fn preview_readiness_probes_children_with_distinct_explicit_client_contexts() {
        ...
        let mut invocation_args = args();
        invocation_args.backend = Some("configured".to_string());
        invocation_args.selector = None;
        let plan = BatchCookFanoutPlan::from_value(
            json!({
                "schema": AGENT_TASK_BATCH_COOK_FANOUT_PLAN_SCHEMA,
                "fanout_id": "configured-preview",
                "cooks": [{
                    "cook_id": "first",
        ...
        .expect("each configured route is ready");

        assert_eq!(evidence["children"].as_array().expect("children").len(), 2);
        assert_eq!(
            std::fs::read_to_string(invoked).expect("readiness count"),
            "2"
        );
    ...
    fn dry_run_planner_uses_one_deadline_across_static_phases_and_marks_live_progress() {
        for _ in 0..10 {
            let args = cook_batch_args();
            let mut planner = DryRunPlanner::new(&args, Placement::Auto);
            // Both phases consult this one absolute deadline before starting a worker.
            planner.deadline = Instant::now() - Duration::from_secs(1);
            let started = Arc::new(AtomicUsize::new(0));

            for (phase, dependency) in [
                ("repository", "registered primary repository"),
                (
                    "issues_and_gates",
                    "supplied issue URLs and gate declarations",
                ),
                ...
                let error = planner
                    .run_bounded(phase, dependency, move || {
                        started.fetch_add(1, Ordering::SeqCst);
                        Ok(())
                    })
                    .expect_err("an expired shared deadline must prevent every static phase");
                assert_eq!(error.details["reason"], "planner_deadline_exceeded");

...
fn resume_recovers_running_action_with_stale_child_run() {
    with_isolated_home(|_| {
        ...
        let plan = test_plan();
        crate::agent_task_lifecycle::submit_plan(&plan, Some("controller-service-stale-child-a"))
            .expect("child submitted");
        crate::agent_task_lifecycle::mark_running("controller-service-stale-child-a")
            .expect("child marked running");
        crate::agent_task_lifecycle::rewrite_record_for_test(

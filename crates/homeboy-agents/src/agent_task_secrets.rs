...
impl AgentTaskSecretConfig {
    ...
    fn path() -> homeboy_core::Result<PathBuf> {
        paths::homeboy().map(|root| root.join("agent-task-secrets.json"))
    }

    fn save(&self) -> homeboy_core::Result<()> {
...
/// Agent-task implementation of core's secret-resolution hook.
struct AgentTaskSecretProviderImpl;

impl homeboy_core::agent_task_secret_provider::AgentTaskSecretProvider
    for AgentTaskSecretProviderImpl
{
    fn resolve_agent_task_secret_env(&self, names: &[String]) -> Vec<(String, String)> {
        resolve_secret_env(names).unwrap_or_default()
    }
}

/// Register the agent-task secret provider so core's trace secret resolution can
/// consult the agent-task secret store without depending on the agent-task
/// subsystem.
pub fn register() {
    homeboy_core::agent_task_secret_provider::register_agent_task_secret_provider(Box::new(
        AgentTaskSecretProviderImpl,
    ));
}

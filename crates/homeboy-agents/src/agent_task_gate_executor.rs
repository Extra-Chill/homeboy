...
fn build_execution(request: &AgentTaskRequest) -> Result<RepoLocalGateExecution> {
    ...
    let cwd = homeboy_core::resolve_contained_local_path(
        workspace_root,
...
fn gate_argv(cwd: &Path, config: &Value, execution_kind: &str) -> Result<Vec<String>> {
    ...
    let script_path = homeboy_core::resolve_contained_local_path(cwd, script, "script")?;
...
fn run_execution(
    ...
    let mut diagnostics = vec![AgentTaskDiagnostic {
        ...
        data: json!({
            ...
            "cwd": execution.cwd,
            "exit_code": exit_code,
            "stdout_tail": crate::agent_task_gate::text_tail(&stdout, 20),
            "stderr_tail": crate::agent_task_gate::text_tail(&stderr, 20),
        }),
    }];
...
fn materialize_output_paths(
    ...
    for (key, declaration) in &execution.artifact_outputs {
        let path = declaration
            ...
            .as_deref()
            .map(|path| {
                homeboy_core::resolve_contained_local_path(
                    &execution.artifact_root,
                    path,

...
mod fanout_batch_read_tests {
    ...
    fn batch_cancel_uses_marker_first_and_replays_the_canonical_ack() {
        with_isolated_home(|_| {
            ...
            .is_err());
            let first = homeboy_core::control_plane::execute_action(
                &requested,
                &request,
                &homeboy_core::control_plane::ControlPlaneInvocationContext::default(),
            )
        ...
        match request.action {
            ControlPlaneAction::Resume => {
                ...
                let result = fanout_batch_resume_action_result(&result.value)
                    .map_err(|error| ControlPlaneError::unavailable(error.message))?;
                let mut action_result = ControlPlaneActionDelegateResult {
                    outcome,
                    result: ControlPlaneActionPayload {
                        schema: CONTROL_PLANE_RESUME_RESULT_SCHEMA.to_string(),
...
impl FanoutBatchDomainService {
    ...
    pub fn status_adjunct(
        ...
        append_child_read_errors(
            &mut report.unavailable_child_runs,
            &report.batch,
            &child_read_errors,
        );
    ...
    fn canonical_children(
        ...
        for child in &batch.child_runs {
            let id = parse_run_id(&child.run_id)?;
...
/// Record canonical child reads that failed as unavailable issues, without
/// duplicating a child the batch projection already reported.
fn append_child_read_errors(
    issues: &mut Vec<AgentTaskBatchChildIssue>,
    batch: &AgentTaskBatchRecord,
    ...
    for child in &batch.child_runs {
        ...
        if issues.iter().any(|issue| issue.run_id == child.run_id) {
            continue;
        }
        issues.push(AgentTaskBatchChildIssue {
...
fn parse_run_id(run_id: &str) -> homeboy_core::Result<RunId> {
            ...
            observation
                .terminalize_control_plane_effect(
                    ...
                    &ControlPlaneEffectTerminal {
                        ...
                        completed_at: recovered.completed_at.clone(),
                        acknowledgement: recovered.clone(),
                        audit: ControlPlaneEffectAudit {
                            schema: CONTROL_PLANE_EFFECT_AUDIT_SCHEMA.to_string(),
                            observed_at: now_timestamp(),
                            evidence: serde_json::json!({
                                "recovery_disposition": "reconciled_expired_effect_lease",
            ...
            action_unavailability(&record, request.action)
        {
            ...
            match request.action {
                ...
                ControlPlaneAction::PlacementUpdate => {
                    let parameters: ControlPlanePlacementUpdateParameters = serde_json::from_value(
                        request.parameters.data.clone(),
                    )
                    .map_err(|error| {
                        ControlPlaneError::invalid_argument(format!(
                    ...
                    let updated = (|| -> homeboy_core::Result<_> {
                        ...
                        let reconciliation =
                            crate::agent_task_service::reconcile_unmaterialized_cook_admission(
                                &resolved,
                            )?;
                    ...
                    match updated {
                        Ok((_updated, current, reconciliation)) => (
                            ...
                            project_record(&current, None)?,
                            ControlPlaneActionPayload {
                                schema: CONTROL_PLANE_PLACEMENT_UPDATE_RESULT_SCHEMA.to_string(),
                ...
                ControlPlaneAction::Retry => {
                    let mut parameters: ControlPlaneRetryParameters = serde_json::from_value(
                        request.parameters.data.clone(),
                    )
                    .map_err(|error| {
                ...
                ControlPlaneAction::Quarantine => {
                    let parameters: ControlPlaneQuarantineParameters = serde_json::from_value(
                        request.parameters.data.clone(),
                    )
                    .map_err(|error| {
                        ControlPlaneError::invalid_argument(format!(
...
mod loop_control_plane_tests {
    ...
    #[test]
    fn loop_stop_cancels_active_work_through_the_real_daemon_boundary() {
        with_isolated_home(|_| {
            super::register();
            crate::agent_task_service::register_work_job_driver();
            crate::agent_task_service::register_loop_work_job_handler();
            ...
            write_controller(&record).expect("persist work identity");

            let (stopped, acknowledgement) =
                stop_loop(loop_id, "active stop").expect("stop active loop");
            assert_eq!(
    ...
    #[test]
    fn loop_stop_cancels_active_work_through_http_action_boundary() {
        with_isolated_home(|_| {
            super::register();
            crate::agent_task_service::register_work_job_driver();
            crate::agent_task_service::register_loop_work_job_handler();
            ...
            write_controller(&record).expect("persist work identity");
            let canonical = control_plane_run_id(loop_id).expect("canonical id");
            let request = homeboy_control_plane_contract::ControlPlaneActionRequest {
                ...
                parameters: homeboy_control_plane_contract::ControlPlaneActionPayload {
                    schema: homeboy_control_plane_contract::CONTROL_PLANE_CANCEL_PARAMETERS_SCHEMA
                        .to_string(),
                    data: json!({ "reason": "active HTTP stop" }),
                },
                confirmed: true,
            };
    ...
    fn loop_stop_uses_canonical_action_ack_and_replays_without_effect() {
        with_isolated_home(|_| {
            ...
            let stale_error = homeboy_core::control_plane::execute_action(
                &canonical,
                &stale,
                &homeboy_core::control_plane::ControlPlaneInvocationContext::default(),
            )
...
mod tests {
    ...
    use homeboy_control_plane_contract::{
        ...
        ControlPlaneOperation, ControlPlaneRef, ControlPlaneReferenceRegistration,
        ControlPlaneReferenceType, ControlPlaneRunListRequest, ControlPlaneRunReviewRequest,
    ...
    fn interrupt_action_after_effect(
        ...
        let accepted_at = super::now_timestamp();
        let intent = ControlPlaneActionIntent {
            schema: CONTROL_PLANE_ACTION_INTENT_SCHEMA.to_string(),
            effect_id: request.effect_id.clone(),
            resource: ControlPlaneActionResource {
            ...
            request: request.clone(),
            request_digest: homeboy_engine_primitives::content_hash::sha256_hex(
                serde_json::to_vec(request)
        ...
        observation
            .lease_control_plane_effect_by_id(
                &request.effect_id,
                "interrupted-test-owner",
                "2000-01-01T00:00:00Z",
                "2000-01-01T00:00:01Z",
            )
            .expect("lease action effect")
            .expect("leased action effect");
    ...
    fn terminal_cancel_is_replayed_and_conflicting_key_reuse_is_rejected() {
        with_isolated_home(|_| {
            ...
            let stale = ControlPlaneActionRequest {
                effect_id: EffectId("test:cancel-request-stale".to_string()),

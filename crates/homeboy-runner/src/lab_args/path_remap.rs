...
#[derive(Debug, Clone)]
pub(crate) struct LabPathRemap {
    pub local: String,
    pub remote: String,
}

impl From<homeboy_core::runner_execution_envelope::PathMaterializationPathRemap> for LabPathRemap {
    fn from(remap: homeboy_core::runner_execution_envelope::PathMaterializationPathRemap) -> Self {
        Self {
...
where
    F: FnMut(&str, &mut Peekable<Iter<'_, String>>, &mut Vec<String>) -> homeboy_core::Result<()>,
{
    let mut out = Vec::with_capacity(args.len());
    let mut iter = args.iter().peekable();
    let mut passthrough = false;
...
pub(crate) fn remap_path_settings_in_args(
    args: &[String],

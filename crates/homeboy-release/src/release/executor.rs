...
use crate::release::{changelog as release_changelog, version};
...
pub(crate) fn step_success(
    ...
    hints: Vec<homeboy_core::error::Hint>,
) -> ReleaseStepResult {
    ReleaseStepResult {
        id: id.to_string(),
...
pub(crate) fn run_git_commit(
    ...
    let status_output =
        homeboy_core::git::status_at(Some(component_id), Some(&component.local_path))?;
    ...
    let output = homeboy_core::git::commit_at(
        Some(component_id),
        Some(&message),
        options,
...
fn load_release_notes(component: &Component) -> Result<String> {
    ...
    let changelog_content = homeboy_core::engine::local_files::local().read(&changelog_path)?;
    validation::require(
...
fn should_amend_release_commit(local_path: &str) -> Result<bool> {
    let log_output =
        homeboy_core::git::execute_git_for_release(local_path, &["log", "-1", "--format=%s"])
    ...
    let status_output = homeboy_core::git::execute_git_for_release(local_path, &["status", "-sb"])
...
mod tests {
    ...
    fn run_package_collects_artifacts_from_multiple_package_providers() {
        homeboy_core::test_support::with_isolated_home(|_| {
            ...
            homeboy_core::extension::catalog::save_manifest(&package_a)
                .expect("save package A extension");
            homeboy_core::extension::catalog::save_manifest(&package_b)
                .expect("save package B extension");
    ...
    fn git_tag_step_refuses_invalid_release_tags() {
        ...
        assert_eq!(result.status, ReleaseStepStatus::Failed);
        assert!(
            !homeboy_core::git::tag_exists_locally(&component.local_path, "v0.6.13").unwrap(),

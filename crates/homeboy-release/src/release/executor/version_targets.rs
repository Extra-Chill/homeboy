...
fn git_toplevel(path: &str) -> Option<std::path::PathBuf> {
    let output =
        homeboy_core::git::execute_git_for_release(path, &["rev-parse", "--show-toplevel"]).ok()?;
...
fn read_version_at_head(component: &Component, target: &VersionTarget) -> Option<String> {
    ...
    let output =
        homeboy_core::git::execute_git_for_release(&component.local_path, &["show", &spec]).ok()?;
    if !output.status.success() {
        return None;
    }

    let content = String::from_utf8(output.stdout).ok()?;
    let normalized_pattern = homeboy_core::component::normalize_version_pattern(&pattern);

...
pub(super) fn persist_release_body(component: &Component, tag: &str, body: &str) -> Option<String> {
    ...
    match std::fs::write(&file, body) {
        ...
        Err(err) => {
            homeboy_core::log_status!("release", "⚠ Could not persist release body: {}", err);
            None
        }
    }
}
...
pub(crate) fn github_changelog_url(
    ...
    let release_scope = crate::release::scope::ReleaseScope::resolve(component, &component.id).ok();
...
pub(crate) fn github_generated_notes_start_tag(
    ...
    let release_scope = crate::release::scope::ReleaseScope::resolve(component, &component.id)?;
    let previous = release_scope.previous_tag_before_any(tag)?;

    if let Some(previous_tag) = previous.as_deref() {
        if !homeboy_core::git::is_ancestor(&release_scope.git_root, previous_tag, tag)? {

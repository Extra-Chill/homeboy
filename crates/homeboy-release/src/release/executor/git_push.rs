...
pub(crate) fn run_git_push(
    ...
    let current_branch = homeboy_core::git::current_branch(std::path::Path::new(
...
fn recover_advanced_remote_push(
    ...
    homeboy_core::git::fetch_origin(path)?;

    let Some(remote_commit) = homeboy_core::git::remote_branch_commit(path, branch)? else {
    ...
    let head_commit = homeboy_core::git::get_head_commit(path)?;
    ...
    if remote_commit == head_commit
        || homeboy_core::git::is_ancestor(path, &remote_commit, &head_commit)?
    {
...
fn push_error_message(output: &homeboy_core::git::GitOutput) -> String {

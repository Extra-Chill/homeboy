...
pub(crate) fn run_git_tag(
    ...
    let output = homeboy_core::git::tag_at(
        Some(component_id),
        Some(tag_name),
        Some(&message),
    ...
    if !output.success {
        ...
        if output.stderr.contains("already exists") {
            let local_exists =
                homeboy_core::git::tag_exists_locally(&component.local_path, tag_name)
                    .unwrap_or(false);
            let remote_exists =
                homeboy_core::git::tag_exists_on_remote(&component.local_path, tag_name)
                    .unwrap_or(false);
...
fn inspect_release_tag_state(component: &Component, tag_name: &str) -> Result<ReleaseTagState> {
    let head_commit = homeboy_core::git::get_head_commit(&component.local_path)?;
    let local_tag_commit = if homeboy_core::git::tag_exists_locally(&component.local_path, tag_name)
        .unwrap_or(false)
    {
        Some(homeboy_core::git::get_tag_commit(
    ...
    let remote_tag_commit = homeboy_core::git::remote_tag_commit(&component.local_path, tag_name)?;
    let github_release = component
        .remote_url
        .clone()
        .or_else(|| {
            homeboy_core::git::release_download::detect_remote_url(std::path::Path::new(
                &component.local_path,
            ))
        })
...
fn build_existing_tag_failure(
    ...
    let mut hints = vec![homeboy_core::error::Hint {
        message: format!(

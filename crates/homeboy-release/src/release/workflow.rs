...
pub fn run_command(input: ReleaseCommandInput) -> Result<(ReleaseCommandResult, i32)> {
    ...
    let options = ReleaseOptions {
        ...
        skip_deps_hydration: input.skip_deps_hydration,
...
fn prepared_tag_publish_recovery_result(
    ...
    if !homeboy_core::git::tag_exists_locally(&release_scope.git_root, &tag)? {
        return Ok(None);
    }
    let tag_commit = homeboy_core::git::get_tag_commit(&release_scope.git_root, &tag)?;
    let head_commit = homeboy_core::git::get_head_commit(&release_scope.git_root)?;
    ...
    if let Some(ref summary) = run.result.summary {
        if !summary.success_summary.is_empty() {
            ...
            for line in &summary.success_summary {
                homeboy_core::log_status!("release", "{}", line);
            }
        }
    }
}
...
pub fn run_batch(
    ...
    for component_id in component_ids {
        ...
        let input = ReleaseCommandInput {
            ...
            skip_deps_hydration: input_template.skip_deps_hydration,
        ...
        match run_command(input) {
            ...
            Err(e) => {
                homeboy_core::log_status!("release", "Failed to release '{}': {}", component_id, e);
                failed += 1;
                results.push(BatchReleaseComponentResult {
                    component_id: component_id.clone(),
    ...
    // Log summary
    if total > 1 {
        homeboy_core::log_status!("release", "--- Batch summary ---");
        homeboy_core::log_status!(
            "release",

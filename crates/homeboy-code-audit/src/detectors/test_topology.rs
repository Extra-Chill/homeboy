...
fn analyze_test_topology(root: &Path) -> Vec<Finding> {
    ...
    for extension in crate::extension_manifests::load_all_audit_manifests() {
...
fn run_topology_script(
    extension: &crate::extension_manifests::AuditExtensionManifest,
    script_rel: &str,
    input: &TopologyInput,
...
fn load_rules(root: &Path) -> Option<TestTopologyRules> {
    let config_path = root.join(homeboy_product_identity::PRODUCT_IDENTITY.config_filename);
...
"#,
        ...
        let extension = crate::extension_manifests::AuditExtensionManifest {

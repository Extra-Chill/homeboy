...
// them so external `crate::X` paths keep working unchanged.
// ============================================================================
...
mod tests {
    ...
    /// Extension-manifest provider registered directly into the audit crate's own
    /// registry so this in-crate test reads the same static instance it registers
    /// into (the dev-dep cycle compiles the audit crate twice — see the artifact-
    ...
    fn register_store_manifest_provider() {
        register_audit_extension_manifest_provider(Box::new(StoreManifestProvider));
    }

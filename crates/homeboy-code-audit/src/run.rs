...
use crate::{baseline, AuditTiming, AuditWithAnalysis, CodeAuditResult};
...
pub struct AuditRunWorkflowArgs {
    ...
    pub only_kinds: Vec<crate::AuditFinding>,
    pub exclude_kinds: Vec<crate::AuditFinding>,
    pub only_labels: Vec<String>,
    pub exclude_labels: Vec<String>,
    pub profile: crate::AuditProfile,
...
fn apply_finding_filters(
    ...
    only_kinds: &[crate::AuditFinding],
    exclude_kinds: &[crate::AuditFinding],
) {
...
fn scope_convention_outliers_to_findings(result: &mut CodeAuditResult) {
    let scoped_findings: HashSet<(String, String, crate::AuditFinding)> = result
        .findings
        .iter()
        .map(|finding| {
            (
...
fn run_audit(args: &AuditRunWorkflowArgs) -> homeboy_error::Result<Option<AuditWithAnalysis>> {
    ...
    if let Some(ref git_ref) = args.changed_since {
        ...
        Ok(Some(crate::audit_path_scoped_with_plan_and_analysis(
            &args.component_id,
            &args.source_path,
            &changed,
            Some(git_ref),
...
fn audit_plan(args: &AuditRunWorkflowArgs) -> crate::AuditExecutionPlan {
    if args.baseline_flags.baseline || args.conventions {
        crate::AuditExecutionPlan::full()
    } else {
        crate::AuditExecutionPlan::from_profile_and_filters(
            args.profile,
            &args.only_kinds,
            &args.exclude_kinds,
        )
...
fn run_baseline_save(
    ...
    let saved = if let Some(ref git_ref) = args.changed_since {
        ...
        baseline::save_baseline_scoped(&result, &changed)
            .map_err(homeboy_error::Error::internal_unexpected)?
    } else {
        baseline::save_baseline(&result).map_err(homeboy_error::Error::internal_unexpected)?
    };

    let baseline_data =
        baseline::load_baseline(Path::new(&result.source_path)).ok_or_else(|| {
            homeboy_error::Error::internal_unexpected("Failed to read back saved baseline")
        })?;
...
fn run_comparison_workflow(
    result: CodeAuditResult,
    analysis: &crate::AuditAnalysisContext,
    args: &AuditRunWorkflowArgs,
...
fn changed_files_for_scope(
    args: &AuditRunWorkflowArgs,
    git_ref: &str,
) -> homeboy_error::Result<Vec<String>> {
    match &args.precomputed_changed_files {
        Some(files) => Ok(files.clone()),
        None => git_changes::get_files_changed_since(&args.source_path, git_ref),
    }
}
...
fn is_blocking_full_audit_finding(finding: &crate::Finding) -> bool {
    !matches!(
        finding.kind,
        crate::AuditFinding::GodFile
            | crate::AuditFinding::HighItemCount
            | crate::AuditFinding::DirectorySprawl
    )
}

...
pub struct AuditBaselineMetadata {
    ...
    /// Legacy scoped policy metadata retained for persisted-baseline reads.
    /// New baselines leave this empty and normalize old section rows into
    /// `known_fingerprints`.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub policy_sections: Vec<AuditBaselinePolicySection>,
}

/// Legacy serialized shape for scoped audit-policy baseline metadata.
///
/// The policy ratchet no longer has a production consumer, but this shape stays
/// readable so existing baselines can migrate explicit section fingerprints to
/// the canonical top-level list.
...
pub struct AuditBaselinePolicySection {
    ...
    /// Legacy predicate metadata. Predicate sections already stored their rows
    /// in the top-level list, so no expansion is needed while loading them.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub fingerprint_prefix: Option<String>,
    /// Explicit fingerprints written by the original section format.
    #[serde(default)]
...
pub(crate) type AuditBaseline = generic::Baseline<AuditBaselineMetadata>;
...
pub(crate) type NewFinding = generic::NewItem;
...
pub(crate) fn save_baseline(result: &CodeAuditResult) -> Result<std::path::PathBuf, String> {
...
pub(crate) fn save_baseline_scoped(
...
pub(crate) fn file_from_audit_fingerprint(fingerprint: &str) -> Option<String> {
...
pub(crate) type BaselinePruneResult = generic::PruneResult;
...
pub(crate) fn compare(result: &CodeAuditResult, baseline: &AuditBaseline) -> BaselineComparison {
    let items: Vec<AuditFinding> = result.findings.iter().map(AuditFinding).collect();
    generic::compare(&items, baseline)
}

/// Return the audit-baseline identity for one finding.
pub(crate) fn finding_baseline_fingerprint(finding: &Finding) -> String {
...
pub(crate) fn baseline_with_reference_findings(
...
fn normalize_loaded_baseline(mut baseline: AuditBaseline) -> AuditBaseline {
    ...
    let section_fingerprints = baseline
        ...
        .policy_sections
        .drain(..)
        .flat_map(|section| section.known_fingerprints)
        .map(normalize_audit_fingerprint)
        .collect::<Vec<_>>();
...
mod tests {
    ...
    use crate::{AuditSummary, CodeAuditResult};
    ...
    fn audit_metadata_roundtrips() {
        ...
        assert!(loaded.metadata.policy_sections.is_empty());

        let _ = std::fs::remove_dir_all(Path::new(&result.source_path));
    }

    #[test]
    fn legacy_policy_section_rows_migrate_to_the_canonical_list() {
        let legacy_fingerprint = "core_boundary_leak:core-agnostic-source::src/core/example.rs::Core boundary leak at line 7::CoreBoundaryLeak".to_string();
        let canonical_fingerprint = normalize_audit_fingerprint(legacy_fingerprint.clone());
        let legacy_section: AuditBaselinePolicySection =
            serde_json::from_value(serde_json::json!({
                "key": "core-boundary",
                "audit_policy": "core_boundary_leak:core-agnostic-source",
                "scope": "core-boundary",
                "issue": "#3498",
                "known_fingerprints": [legacy_fingerprint],
            }))
            .expect("legacy policy section");
        let baseline = AuditBaseline {
            created_at: "2026-06-04T00:00:00Z".to_string(),
            context_id: "test".to_string(),
            item_count: 1,
            known_fingerprints: vec!["structural::src/core/other.rs::LargeFile".to_string()],
            metadata: AuditBaselineMetadata {
                outliers_count: 1,
                alignment_score: None,
                known_outliers: vec![],
                policy_sections: vec![legacy_section],
            },
        };

        let normalized = normalize_loaded_baseline(baseline);

        assert_eq!(normalized.item_count, 2);
        assert!(normalized
            .known_fingerprints
            .contains(&canonical_fingerprint));
        assert!(normalized.metadata.policy_sections.is_empty());
    }

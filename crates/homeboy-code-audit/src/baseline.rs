...
///
/// A section's membership can be expressed two ways, and both are honored:
///
/// * `fingerprint_prefix` — a predicate evaluated against the baseline's own
///   `known_fingerprints`. This is the serialized form written today.
/// * `known_fingerprints` — an explicit enumeration. This is the legacy form.
///   It is still read so pre-migration baselines keep loading, and it is still
///   additive so a section can name a row the predicate would not select.
///
/// The enumeration was previously the *only* form, which made the section a
/// byte-for-byte re-listing of rows already present at the top level (678 of
/// them in this repo's own baseline). Since `policy_sections_from_fingerprints`
/// regenerates the section from the top-level array on every save, those rows
/// were generated data, not hand-maintained data — so the serialized form can
/// state the predicate and let the reader derive the same effective set.
...
pub struct AuditBaselinePolicySection {
    ...
    /// Predicate selecting this section's rows from the baseline's own
    /// `known_fingerprints` instead of re-listing them.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub fingerprint_prefix: Option<String>,
    /// Explicitly enumerated fingerprints for this policy/scope section.
    ///
    /// `#[serde(default)]` is load-bearing: without it, dropping the key from a
    /// baseline fails deserialization of the *whole* baseline, and every
    /// recorded row silently reads as a brand-new finding.
    ///
    /// It is deliberately **not** `skip_serializing_if`. A homeboy binary older
    /// than this field's `#[serde(default)]` still reads the baselines this
    /// binary writes — `homeboy-action` falls back to the last released binary
    /// when a source build fails — and omitting the key would fail that
    /// binary's deserialization outright. Emitting an empty array costs one
    /// line, resolves to the same effective set on both binaries, and removes
    /// the transition window entirely.
    #[serde(default)]
    pub known_fingerprints: Vec<String>,
}

impl AuditBaselinePolicySection {
    /// Rows this section covers: the explicit enumeration plus every baseline
    /// fingerprint selected by `fingerprint_prefix`.
    ///
    /// Both forms produce the same effective set for a section generated by
    /// [`policy_sections_from_fingerprints`], because that generator selected
    /// the enumeration with the same prefix test.
    #[allow(
        dead_code,
        reason = "no production caller; exercised by this crate's tests"
    )]
    pub(crate) fn effective_fingerprints<'a>(
        &'a self,
        baseline_fingerprints: &'a [String],
    ) -> BTreeSet<&'a str> {
        ...
        if let Some(prefix) = self.fingerprint_prefix.as_deref() {
            fingerprints.extend(
...
///
/// Only `stale_policy_baseline_fingerprints` consumes this, and that path has
/// no production caller — see the allow there.
#[allow(
    dead_code,
    reason = "only reachable through test-only baseline helpers"
)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum PolicyBaselineMode {
...
pub(crate) const SOURCE_POLICY_CORE_BOUNDARY_POLICY: &str =
    "core_boundary_leak:core-agnostic-source";
pub(crate) const SOURCE_POLICY_CORE_BOUNDARY_SCOPE: &str = "core-boundary";
...
pub(crate) fn policy_baseline_fingerprints<'a>(
    ...
    {
        fingerprints.extend(section.effective_fingerprints(&baseline.known_fingerprints));
    }
...
pub(crate) fn stale_policy_baseline_fingerprints(
    baseline: &AuditBaseline,
...
/// Emit the scoped policy sections for a baseline.
///
/// The section states the predicate that selects its rows rather than
/// re-listing rows the baseline already carries at the top level. Readers go
/// through [`AuditBaselinePolicySection::effective_fingerprints`], so the
/// effective set is identical to the enumeration this used to write.
fn policy_sections_from_fingerprints(fingerprints: &[String]) -> Vec<AuditBaselinePolicySection> {
    let has_core_boundary = fingerprints
        .iter()
        .any(|fingerprint| fingerprint.starts_with(SOURCE_POLICY_CORE_BOUNDARY_POLICY));

    if !has_core_boundary {
        return Vec::new();
    }
    ...
    vec![AuditBaselinePolicySection {
        ...
        fingerprint_prefix: Some(SOURCE_POLICY_CORE_BOUNDARY_POLICY.to_string()),
        known_fingerprints: Vec::new(),
    }]
}
...
mod tests {
    ...
    #[test]
    fn saved_policy_section_states_a_predicate_instead_of_re_listing_rows() {
        let result = make_result(
            vec![make_finding_with_kind(
                SOURCE_POLICY_CORE_BOUNDARY_POLICY,
                "src/core/example.rs",
            ...
            "policy_section_predicate",
        );

        let _ = save_baseline(&result).unwrap();
        let loaded = load_baseline(Path::new(&result.source_path)).unwrap();
        let section = &loaded.metadata.policy_sections[0];

        assert_eq!(
            section.fingerprint_prefix.as_deref(),
            Some(SOURCE_POLICY_CORE_BOUNDARY_POLICY),
            "a newly saved section states its membership predicate"
        );
        assert!(
            section.known_fingerprints.is_empty(),
            "a newly saved section does not re-list rows the baseline already carries"
        );
        let serialized = serde_json::to_value(section).expect("section serializes");
        assert_eq!(
            serialized["known_fingerprints"].as_array().map(Vec::len),
            Some(0),
            "the key stays present and empty so a pre-migration binary can still read it"
        );
        assert_eq!(
            section.effective_fingerprints(&loaded.known_fingerprints),
            loaded
                .known_fingerprints
                .iter()
                .map(String::as_str)
                .collect::<BTreeSet<_>>(),
            "the predicate expands to the same rows the enumeration used to list"
        );

        let _ = std::fs::remove_dir_all(Path::new(&result.source_path));
    }

    /// The migration has to be safe in both directions: a baseline written
    /// before the predicate existed must still deserialize *and* resolve to the
    /// same effective set as one written after it. Without `#[serde(default)]`
    /// on `known_fingerprints`, the enumerated-form drop fails the whole
    /// baseline and every recorded row reads as a brand-new finding.
    #[test]
    fn enumerated_and_predicate_sections_resolve_to_the_same_effective_set() {
        let fingerprint = format!(
            "{}::src/core/example.rs::Core boundary leak (core-agnostic-source) configured ecosystem term `ProductName` appears at line <line> in behavioral context `top-level`::CoreBoundaryLeak",
            SOURCE_POLICY_CORE_BOUNDARY_POLICY
        );
        let unrelated = "structural::src/core/other.rs::LargeFile".to_string();

        let legacy: AuditBaselinePolicySection = serde_json::from_value(serde_json::json!({
            "key": format!("{SOURCE_POLICY_CORE_BOUNDARY_POLICY}/{SOURCE_POLICY_CORE_BOUNDARY_SCOPE}"),
            "audit_policy": SOURCE_POLICY_CORE_BOUNDARY_POLICY,
            "scope": SOURCE_POLICY_CORE_BOUNDARY_SCOPE,
            "known_fingerprints": [fingerprint.clone()],
        }))
        .expect("a pre-migration enumerated section still deserializes");

        let migrated: AuditBaselinePolicySection = serde_json::from_value(serde_json::json!({
            "key": format!("{SOURCE_POLICY_CORE_BOUNDARY_POLICY}/{SOURCE_POLICY_CORE_BOUNDARY_SCOPE}"),
            "audit_policy": SOURCE_POLICY_CORE_BOUNDARY_POLICY,
            "scope": SOURCE_POLICY_CORE_BOUNDARY_SCOPE,
            "fingerprint_prefix": SOURCE_POLICY_CORE_BOUNDARY_POLICY,
        }))
        .expect("a post-migration section with no enumeration deserializes");

        let top_level = vec![fingerprint.clone(), unrelated];

        assert_eq!(
            legacy.effective_fingerprints(&top_level),
            migrated.effective_fingerprints(&top_level)
        );
        assert_eq!(
            migrated.effective_fingerprints(&top_level),
            BTreeSet::from([fingerprint.as_str()]),
            "the predicate selects only rows owned by this policy"
        );
    }

    #[test]
    fn predicate_section_survives_a_save_load_save_cycle() {
        let result = make_result(
            vec![make_finding_with_kind(
                SOURCE_POLICY_CORE_BOUNDARY_POLICY,
                "src/core/example.rs",
            ...
            "policy_section_resave",
        );

        let _ = save_baseline(&result).unwrap();
        let first = load_baseline(Path::new(&result.source_path)).unwrap();
        let _ = save_baseline(&result).unwrap();
        let second = load_baseline(Path::new(&result.source_path)).unwrap();

        assert_eq!(first.known_fingerprints, second.known_fingerprints);
        assert_eq!(
            first.metadata.policy_sections,
            second.metadata.policy_sections
        );
        assert_eq!(
            policy_baseline_fingerprints(
                &first,
                SOURCE_POLICY_CORE_BOUNDARY_POLICY,
                Some(SOURCE_POLICY_CORE_BOUNDARY_SCOPE),
            ),

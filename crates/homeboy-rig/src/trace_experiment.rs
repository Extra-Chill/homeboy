...
pub fn resolve_settings(
    ...
    experiment: &crate::TraceExperimentSpec,
...
pub fn run_phase(
    ...
    experiment_name: &str,
    phase: &str,
    commands: &[crate::TraceExperimentCommandSpec],
...
pub fn collect_artifacts(
    ...
    for (index, artifact) in experiment.artifacts.iter().enumerate() {
        let (label, source) = match artifact {
            crate::TraceExperimentArtifactSpec::Path(path) => (
                Path::new(path)
                    .file_name()
            ...
            crate::TraceExperimentArtifactSpec::Detailed { label, path } => {
...
/// needs.
pub fn prepare_experiment_bundle_dir(dir: &Path) -> homeboy_core::Result<()> {
...
/// `mkdir -p`.
pub fn prepare_experiment_overlay_dir(dir: &Path) -> homeboy_core::Result<()> {
    std::fs::create_dir_all(dir).map_err(|err| {
        homeboy_core::Error::internal_io(
            format!(
...
pub fn read_experiment_overlay(source: &Path) -> homeboy_core::Result<Vec<u8>> {
...
pub fn write_experiment_overlay(target: &Path, bytes: &[u8]) -> homeboy_core::Result<()> {
...
pub fn read_experiment_overlay_for_checksum(path: &Path) -> homeboy_core::Result<Vec<u8>> {

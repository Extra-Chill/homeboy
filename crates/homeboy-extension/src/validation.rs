...
pub fn validate_required_extensions(component: &homeboy_core::component::Component) -> Result<()> {
    let extensions = match &component.extensions {
        Some(m) if !m.is_empty() => m,
        _ => return Ok(()),
    };

    let mut missing: Vec<(&String, &homeboy_core::component::ScopedExtensionConfig)> = Vec::new();
    ...
    let mut err = homeboy_core::error::Error::new(
        homeboy_core::error::ErrorCode::ExtensionNotFound,
        message,
        serde_json::json!({
...
fn extension_install_hint(
    extension_id: &str,
    ext_config: &homeboy_core::component::ScopedExtensionConfig,
) -> String {
...
fn extension_source(ext_config: &homeboy_core::component::ScopedExtensionConfig) -> Option<&str> {

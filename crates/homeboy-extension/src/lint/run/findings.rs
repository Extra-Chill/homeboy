...
use homeboy_core::finding::{FindingProducerSummary, FindingSource, HomeboyFinding};
...
pub(super) fn parse_lint_producer_summaries_file(
    path: &Path,
) -> homeboy_core::Result<Vec<FindingProducerSummary>> {
    fn parse_error(path: &Path, error: std::io::Error) -> homeboy_core::Error {
    ...
    fn json_error(path: &Path, error: serde_json::Error) -> homeboy_core::Error {
        homeboy_core::Error::internal_io(
            format!(

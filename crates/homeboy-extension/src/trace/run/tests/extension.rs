//! Extension-resolved trace runner, list discovery, and overlay tests.
...
use super::super::workflow::run_trace_workflow_with_context;
...
fn test_build_trace_runner() {
    let _home_env_guard = homeboy_core::test_support::home_env_guard();
...
"#,
    ...
    let output = build_trace_runner(Some(&context), &component, &args, &run_dir, false).unwrap();
...
fn test_trace_is_unclaimed() {
    ...
    let other = Error::validation_invalid_argument("extension", "different problem", None, None);
...
"#,
    ...
    let output = run_trace_workflow_with_context(Some(&context), &component, args, &run_dir, None)
...
"#,
    ...
    assert_eq!(
        output
            .results
            .as_ref()
            .and_then(|results| results.summary.as_deref()),

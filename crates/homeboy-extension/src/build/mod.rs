...
// artifact-path resolution relocated to homeboy_core::build_artifact_path (pure core glue)

pub use homeboy_core::build_artifact_path::{
    resolve_artifact_path, resolve_artifact_path_from_root,
};
...
fn execute_build_component(
    ...
    if let Some(gap) = homeboy_core::tag_gap::detect_tag_gap(comp) {
        homeboy_core::tag_gap::warn_tag_gap(&comp.id, &gap, "build");
...
impl homeboy_core::component_build_provider::ComponentBuildRunner
    for ExtensionComponentBuildRunner
{
    fn run_component_build(
        ...
        let value = serde_json::to_value(&result).map_err(|e| {
            homeboy_core::Error::internal_json(
                e.to_string(),
                Some("serialize build result".to_string()),
            )
        })?;
        Ok((value, exit_code))
    }

    fn can_build(&self, component: &homeboy_core::component::Component) -> bool {
...
pub fn register_component_build_runner() {
    homeboy_core::component_build_provider::register_component_build_runner(Box::new(

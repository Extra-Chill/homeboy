...
pub fn detect_test_drift(
    ...
    component: &Component,
    since: &str,
) -> Result<DriftWorkflowResult, homeboy_core::Error> {
    ...
    if report.production_changes.is_empty() {
        homeboy_core::log_status!("drift", "No production changes detected since {}", since);
    } else {
        homeboy_core::log_status!(
            "drift",
...
pub fn auto_fix_test_drift(
    ...
    write: bool,
    include_report: bool,
) -> Result<AutoFixDriftWorkflowResult, homeboy_core::Error> {
    let source_path = {
    ...
    let output = if rules.is_empty() {
        homeboy_core::log_status!("test", "No auto-fixable drift detected. Nothing to apply.");
        ...
        let summary = homeboy_core::refactor_transform_provider::apply_transform_set(
            &source_path,
            "test_auto_fix_drift",
            &set,
            write,

...
pub struct ImportRef {
    ...
    pub line: usize,
    /// The raw module path from the import (e.g., `crate::fixer`).
...
///
/// `"crate::fixer::module_path_from_file"` → (`"crate::fixer"`, `["module_path_from_file"]`)
/// `"crate::fixer::{insertion, Fix}"` → (`"crate::fixer"`, `["insertion", "Fix"]`)
...
///
/// Handles both absolute (`crate::fixer`) and the import's
...
fn load_grammar_for_ext(ext: &str) -> Option<Grammar> {
    let matched = crate::extension::catalog::find_extension_for_file_ext(ext, "fingerprint")?;
...
mod tests {
    ...
    fn parse_rust_import_simple() {
        let (module, names) = parse_rust_import_path("crate::fixer::module_path_from_file");
        assert_eq!(module, "crate::fixer");
        assert_eq!(names, vec!["module_path_from_file"]);
    }

    #[test]
    fn parse_rust_import_grouped() {
        let (module, names) = parse_rust_import_path("crate::fixer::{insertion, Fix}");
        assert_eq!(module, "crate::fixer");
    ...
    fn parse_rust_import_self() {
        // `use crate::fixer::self;` — the whole path is the module, no named imports
        let (module, names) = parse_rust_import_path("crate::fixer::self");
        assert_eq!(module, "crate::fixer::self");
    ...
    fn compute_rewrite_simple_import() {
        let import = ImportRef {
            ...
            line: 22,
            module_path: "crate::refactor::move_items".to_string(),
            imported_names: vec!["walk_source_files".to_string()],
            original_text: "use crate::refactor::move_items::walk_source_files;".to_string(),
        };
    ...
    fn compute_rewrite_grouped_import() {
        let import = ImportRef {
            ...
            line: 5,
            module_path: "crate::fixer".to_string(),
            imported_names: vec![
                "insertion".to_string(),
                "module_path_from_file".to_string(),
                "Fix".to_string(),
            ],
            original_text: "use crate::fixer::{insertion, module_path_from_file, Fix};".to_string(),
        };
        ...
        assert!(rewrite
            .replacement
            .contains("use crate::fixer::{insertion, Fix};"));
        assert!(rewrite
            .replacement
            .contains("use crate::symbol_graph::module_path_from_file;"));
    }
}

...
const _: () = {
    assert!(str_eq(
        crate::lab_contract::RUNNER_ARTIFACT_MANIFEST_FILE,
...
impl ArtifactManifest {
    ...
    pub fn validate_under(
        ...
        for entry in &self.artifacts {
            ...
            let actual_sha256 = crate::artifact_metadata::sha256_file(&absolute_path)?;
            ...
            if entry.content_type.is_none() {
                entry.content_type =
                    crate::artifact_metadata::content_type_from_path(&absolute_path);
            }
...
impl ArtifactManifestEntry {
    pub(crate) fn to_artifact_contract(
        &self,
    ) -> Result<crate::artifact_contract::ArtifactContract> {
        ...
        Ok(crate::artifact_contract::ArtifactContract {
            schema: crate::artifact_contract::ARTIFACT_CONTRACT_SCHEMA.to_string(),
    ...
    for entry in fs::read_dir(current).map_err(|e| {
        ...
        if metadata.is_dir() {
            ...
            artifacts.push(ArtifactManifestEntry {
                ...
                label: None,
                semantic_key: None,
                content_type: crate::artifact_metadata::content_type_from_path(&path),
                provenance: None,
                viewer: None,
                public_url: None,
                public_url_state: None,
                size_bytes: Some(metadata.len()),
                sha256: Some(crate::artifact_metadata::sha256_file(&path)?),
                redaction: None,
...
mod tests {
    ...
    fn validates_descriptor_fields_and_projects_artifact_contracts() {
        ...
        assert_eq!(
            contract.schema,
            crate::artifact_contract::ARTIFACT_CONTRACT_SCHEMA
        );

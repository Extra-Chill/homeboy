...
enum LiveCancellationOutcome {
    Terminated(crate::process::ProcessTreeTermination),
    RunnerJobCancelled {
...
fn classify_live_cancellation(record: &AgentTaskRunRecord) -> Result<LiveCancellationOutcome> {
    ...
    if record.is_runner_backed() {
        ...
        if let Some(pid) = owner_pid {
            recovery_commands.extend(crate::process::process_tree_recovery_commands(pid));
        }
    ...
    if let Some(pid) = owner_pid {
        return Ok(LiveCancellationOutcome::Unsupported(
            UnsupportedLiveCancellation {
                ...
                runner_id: None,
                runner_job_id: None,
                recovery_commands: crate::process::process_tree_recovery_commands(pid),
            },
        ));
    }
...
pub(super) mod test_cancel_hook {
    ...
    type CancelHook = Box<
        dyn FnMut(&str, &str) -> Result<(crate::api_jobs::Job, Vec<crate::api_jobs::JobEvent>)>,
    >;
    ...
    pub(super) fn take(
        runner_id: &str,
        runner_job_id: &str,
    ) -> Option<Result<(crate::api_jobs::Job, Vec<crate::api_jobs::JobEvent>)>> {
        HOOK.with(|cell| {
            cell.borrow_mut()
                .as_mut()

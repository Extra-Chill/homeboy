...
use crate::defaults::{self, AgentTaskSecretSource};
use crate::keychain;
use crate::paths;
use crate::secret_env_plan::{resolve_secret_env_names, SecretEnvPlan, SecretEnvValueProvider};
use crate::Error;
use base64::Engine;
...
pub fn map_secret_to_env(
    name: &str,
    env_var: Option<&str>,
) -> crate::Result<AgentTaskSecretEnvStatus> {
...
pub fn set_config_secret(name: &str, value: &str) -> crate::Result<AgentTaskSecretEnvStatus> {
    let mut config = AgentTaskSecretConfig::load();
    config.secrets.insert(
        name.to_string(),
        AgentTaskSecretSource {

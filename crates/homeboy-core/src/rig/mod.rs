...
use crate::extension::bench::parsing::{RigPackageEvidence, RigPackageFreshness};
use crate::{git, paths};
...
fn read_spec_from_path(
    ...
    spec.id = crate::extension::slugify_id(&spec.id)?;
...
pub fn load_local_source(source: &str, id: Option<&str>) -> Result<RigSpec> {
    ...
    if let Some(id) = id {
        let id = crate::extension::slugify_id(id)?;
        if rigs.is_empty() {
            return Err(Error::validation_invalid_argument(
                "id",

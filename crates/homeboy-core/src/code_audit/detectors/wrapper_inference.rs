...
pub(in crate::code_audit) fn run(fingerprints: &[&FileFingerprint], root: &Path) -> Vec<Finding> {
...
fn load_config(root: &Path) -> Option<WrapperInferenceConfig> {
    let config_path = root.join(crate::product_identity::PRODUCT_IDENTITY.config_filename);

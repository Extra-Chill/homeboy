...
use crate::{config, defaults, worktree, worktree_providers, Error, Result};
...
mod tests {
    ...
    use crate::agent_task_lifecycle::{self as lifecycle, AgentTaskRunState};
    use crate::agent_task_prompts;
    ...
    fn submitted_provider_policy_preserves_controller_execution_policy() {
        with_isolated_home(|_| {
            ...
            let runner_catalog = crate::agent_task_provider::AgentTaskProviderCatalog {
                        ...
                        rotation: Some(
                            crate::agent_task_scheduler::AgentTaskProviderRotationPolicy {
                                entries: ["model-one", "model-two", "model-three"]
                                    .into_iter()
                                    .map(|model| crate::agent_task_scheduler::AgentTaskProviderRotationEntry {
    ...
    fn builds_dispatch_plan_from_stored_prompt_ref() {
        with_isolated_home(|_| {
            crate::agent_task_prompts::save_prompt(
                "review-fix",
    ...
    fn dispatch_plan_promotes_runtime_component_contracts_from_client_context_inputs() {
        ...
        assert_eq!(
            graph["schema"],
            crate::agent_task_runtime_dependency_graph::RUNTIME_DEPENDENCY_GRAPH_SCHEMA
        );

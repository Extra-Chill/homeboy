...
//!
//! - Bot identity and commit prefix: [`crate::git`] / [`super::guard`].
//! - Drift-file classification: [`crate::component::drift`].
//! - Git primitives: [`crate::git`] (`stage_all`, `commit_*`, `run_git`).
//!
...
pub fn run_autofix_transaction(request: TransactionRequest<'_>) -> Result<TransactionOutcome> {
    ...
    let drift_files = crate::component::drift::drift_file_paths(request.component);

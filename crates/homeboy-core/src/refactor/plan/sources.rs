...
use crate::git;
use crate::refactor::auto::FixResultsSummary;
use crate::Error;
use serde::Serialize;
...
pub struct LintSourceOptions {
    ...
    pub sniff_filters: crate::extension::lint::LintSniffFilters,
...
pub fn collect_refactor_sources(
    ...
    if applied {
        ...
        require_successful_format(
            crate::engine::format_write::format_after_write(&request.root, &abs_changed)?,
...
fn clean_homeboy_owned_ci_artifacts(root: &Path) -> crate::Result<bool> {
...
fn dirty_changes_are_only_homeboy_artifacts(changes: &crate::git::UncommittedChanges) -> bool {
    changes
        .staged
        .iter()
...
fn ensure_clean_enough_for_write(
    request: &RefactorSourceRequest,
    changes: Option<&crate::git::UncommittedChanges>,
) -> crate::Result<()> {
...
mod tests {
    ...
    fn formatter_failure_is_a_write_failure() {
        let result = require_successful_format(
            crate::engine::format_write::FormatResult {
                success: false,
    ...
    fn lint_fix_validation_rejects_remaining_findings() {
        let findings = vec![
            crate::finding::HomeboyFinding::builder("lint", "Tabs must be used")

...
use crate::refactor::auto::{self, FixApplied};
use crate::refactor::plan::verify::AuditConvergenceScoring;
use crate::Error;
...
pub(super) fn format_changed_files(root: &Path, changed_files: &[String]) -> crate::Result<()> {
    ...
    let fmt = crate::engine::format_write::format_after_write(root, &abs_changed)?;
...
pub(super) fn require_successful_format(
    fmt: crate::engine::format_write::FormatResult,
    label: &str,
) -> crate::Result<()> {
    if fmt.success {
        return Ok(());
...
pub(super) fn reject_remaining_lint_fix_findings(
    findings: &[crate::finding::HomeboyFinding],
) -> crate::Result<()> {
...
pub(super) fn plan_audit_stage(request: AuditStageRequest<'_>) -> crate::Result<PlannedStage> {
    ...
    let mut fix_result =
        crate::refactor::plan::generate::generate_audit_fixes(&result, root, &policy);
    let (fix_result, policy_summary, changed_files, stage_warnings): (
        ...
        let outcome = crate::refactor::plan::verify::run_audit_refactor(
            result.clone(),
            request.only,
            request.exclude,
    ...
    let lint_findings = if let Some(CachedLintResult::HasFindings(count)) = cached {
        ...
        let cached_findings = output_dir
            .and_then(|dir| {
                ...
                let findings: Vec<crate::finding::HomeboyFinding> =
    ...
    let (stage_changed_files, fix_results, stage_warnings) = if write && !lint_findings.is_empty() {
        ...
        if !scope_outcome.changed_files.is_empty() {
            ...
            let remaining_findings =
                crate::extension::lint::baseline::parse_findings_file(&findings_file)?;

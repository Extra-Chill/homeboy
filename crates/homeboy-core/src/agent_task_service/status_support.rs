...
fn hydrate_local_path_evidence_ref(path: &Path) -> Result<HydratedContent> {
    let metadata =
        fs::metadata(path).map_err(|error| crate::Error::internal_io(error.to_string(), None))?;
    if !metadata.is_file() {
        return Err(crate::Error::validation_invalid_argument(
            "evidence_ref",
    ...
    let bytes =
        fs::read(path).map_err(|error| crate::Error::internal_io(error.to_string(), None))?;
...
fn parse_agent_task_homeboy_uri(uri: &str) -> Result<ParsedAgentTaskUri> {
    ...
    let run_id =
        crate::execution_contract::decode_uri_component_strict(run_id).ok_or_else(|| {
            crate::Error::validation_invalid_argument(
                "evidence_ref",
...
fn fragment_value(fragment: &str, key: &str, uri: &str) -> Result<Option<String>> {
    for part in fragment.split('&') {
        ...
        if candidate == key && !value.is_empty() {
            return crate::execution_contract::decode_uri_component_strict(value)
                .map(Some)
                .ok_or_else(|| {
                    crate::Error::validation_invalid_argument(
                        "evidence_ref",

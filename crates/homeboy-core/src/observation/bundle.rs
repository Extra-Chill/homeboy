...
pub fn build_bundle(
    ...
    runs: Vec<RunRecord>,
) -> crate::Result<ObservationBundle> {
...
pub fn write_bundle_dir(path: &Path, bundle: &ObservationBundle) -> crate::Result<()> {
...
pub fn read_bundle_dir(path: &Path) -> crate::Result<ObservationBundle> {
    if !path.is_dir() {
        return Err(Error::validation_invalid_argument(
            "input",
...
fn zip_directory_artifact(path: &Path) -> crate::Result<Vec<u8>> {
...
fn sorted_directory_files(path: &Path) -> crate::Result<Vec<PathBuf>> {
...
fn collect_directory_files(path: &Path, files: &mut Vec<PathBuf>) -> crate::Result<()> {
...
fn write_json(path: PathBuf, value: &impl Serialize) -> crate::Result<()> {
...
fn read_json<T: for<'de> Deserialize<'de>>(path: PathBuf) -> crate::Result<T> {
    crate::config::read_json_file_with(
        &path,
        |e| {
            Error::internal_io(
                e.to_string(),
...
fn read_optional_json<T: for<'de> Deserialize<'de> + Default>(path: PathBuf) -> crate::Result<T> {
    if !path.exists() {
        return Ok(T::default());
    }
    read_json(path)

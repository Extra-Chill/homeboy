...
use crate::finding::{FindingProducer, FindingSource, HomeboyFinding};
...
impl From<FindingRecord> for RecordedHomeboyFinding {
    fn from(record: FindingRecord) -> Self {
        ...
        let finding = HomeboyFinding {
            ...
            location: crate::finding::FindingLocation {
                file: record.file,
                line: record.line,
                column,
            },
...
fn homeboy_findings_from_annotation_file(path: &Path) -> crate::error::Result<Vec<HomeboyFinding>> {
    if !path.exists() {
        return Ok(Vec::new());
    }

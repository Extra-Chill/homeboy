use crate::engine::resource::ResourceSummaryRun;
...
pub trait WorkflowObservationAdapter<T> {
    ...
    fn error_metadata(&self, _error: &crate::Error) -> Option<serde_json::Value> {
        None
    }
}
...
impl BestEffortObservedRun {
    ...
    fn warn(&mut self, operation: &'static str, error: crate::Error) {
...
impl ObservedWorkflowRunner {
    pub fn create(resource_label: impl Into<String>) -> crate::Result<Self> {
        Ok(Self {
...
mod tests {
    ...
    impl WorkflowObservationAdapter<i32> for FixtureAdapter {
        ...
        fn error_metadata(&self, error: &crate::Error) -> Option<serde_json::Value> {
    ...
    fn latest_fixture_run() -> crate::observation::RunRecord {
    ...
    fn observed_workflow_runner_finishes_error_after_resource_summary() {
        ...
        let runner = ObservedWorkflowRunner::create("test demo").expect("runner");
        let resource_summary_path = runner.run_dir().step_file("resource-summary.json");
        let error = crate::Error::validation_invalid_argument(
            "fixture",
    ...
    fn adapted_observed_workflow_finishes_success_and_records_findings() {
        with_isolated_home(|_| {
            ...
            let findings = ObservationStore::open_initialized()
                .expect("store")
                .list_findings(crate::observation::FindingListFilter {
                    run_id: Some(run.id),
                    tool: Some("fixture".to_string()),
                    ..crate::observation::FindingListFilter::default()
                })
                .expect("findings");

...
use crate::plan::PlanStepDependencyKind;
...
pub(super) fn controller_spec_homeboy_plan(
    ...
    plan.summary = Some(crate::plan::PlanSummary {
...
pub(crate) fn compile_executable_plan_from_spec(
    ...
    plan.summary = Some(crate::plan::PlanSummary::from_steps(&plan.steps));
    Ok(plan)
}
    ...
    if fan_out_config.entity_ids.is_empty() && fan_out_config.dynamic_artifact.is_none() {
        ...
        Ok(AgentTaskLoopPolicyAction::FanOut {
            ...
            max_items: fan_out
                ...
                .unwrap_or(crate::agent_task_loop_controller::DEFAULT_FAN_OUT_MAX_ITEMS),
            fail_fast: fan_out

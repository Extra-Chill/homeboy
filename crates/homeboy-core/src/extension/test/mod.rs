...
fn test_passthrough_filter(
    component: &Component,
) -> crate::error::Result<Option<TestPassthroughFilter>> {
    let context = resolve_test_command(component)?;
    let manifest = crate::extension::load_extension(&context.extension_id)?;
    Ok(manifest
        .test
...
fn test_changed_file_routing(
    extension_id: &str,
) -> crate::error::Result<Option<TestChangedFileRouting>> {
...
pub fn resolve_drift_options(
    ...
    Ok(DriftOptions::from_config(
        &source_path,
        since,
        &crate::defaults::extension_provided_test_drift_config(),
        &[],
    ))
}
...
fn is_direct_changed_test_path(file: &str) -> bool {
    ...
    crate::defaults::extension_provided_direct_test_file_suffixes()
        .iter()

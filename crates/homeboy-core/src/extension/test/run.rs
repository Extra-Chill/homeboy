...
fn run_review_test_lifecycle(
    ...
    component: String,
    json_summary: bool,
    run: impl FnOnce() -> crate::Result<TestRunWorkflowResult>,
) -> crate::Result<TestRunWorkflowResult> {
...
impl TestCheckoutGuard {
    fn capture(path: &Path) -> crate::Result<Self> {
        let changes = crate::git::get_uncommitted_changes(&path.to_string_lossy())?;
        ...
        let head = crate::git::run_git(path, &["rev-parse", "HEAD"], "capture review test HEAD")?;
        Ok(Self {
    ...
    fn restore(&self) -> crate::Result<()> {
        ...
        let changes = crate::git::get_uncommitted_changes(&self.path.to_string_lossy())?;
        if changes.has_changes {
...
fn parse_declared_parser_stdout_json(stdout: &str) -> crate::Result<TestCounts> {
    ...
    let count = |field: &str| -> crate::Result<u64> {
        object
            .get(field)
...
pub fn run_self_check_test_workflow_with_progress(
    ...
    observation: Option<&crate::observation::ActiveObservation>,
) -> crate::Result<TestRunWorkflowResult> {
...
"#,
            ...
            let context = crate::extension::ExtensionExecutionContext {

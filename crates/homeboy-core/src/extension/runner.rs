...
impl ExtensionRunner {
    ...
    pub fn with_run_dir(mut self, run_dir: &crate::engine::run_dir::RunDir) -> Self {
        ...
        self.env_vars.push((
            crate::server::DELEGATED_RUN_STATUS_FILE_ENV.to_string(),
            run_dir
...
pub(in crate::extension) fn tail_lines(s: &str, max_lines: usize) -> (String, bool) {
...
mod tests {
    ...
    fn with_run_dir_tracks_resource_artifact_path() {
        ...
        assert!(runner.env_vars.iter().any(|(key, value)| key
            == crate::server::DELEGATED_RUN_STATUS_FILE_ENV
            && value
                == &run_dir
                    .step_file("delegated-run-status.json")
                    .to_string_lossy()));

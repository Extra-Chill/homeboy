...
use crate::finding::{FindingProducerSummary, FindingSource, HomeboyFinding};
...
pub(super) fn parse_lint_producer_summaries_file(
    path: &Path,
) -> crate::Result<Vec<FindingProducerSummary>> {
    fn parse_error(path: &Path, error: std::io::Error) -> crate::Error {
    ...
    fn json_error(path: &Path, error: serde_json::Error) -> crate::Error {
        crate::Error::internal_io(
            format!(

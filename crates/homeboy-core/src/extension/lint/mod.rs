...
fn write_changed_files_manifest(
    ...
    let manifest_path = run_dir.step_file(crate::engine::run_dir::files::LINT_CHANGED_FILES);
...
mod tests {
    ...
    fn changed_files_manifest_unset_when_no_list_supplied() {
        ...
        assert!(
            !run_dir
                .step_file(crate::engine::run_dir::files::LINT_CHANGED_FILES)
                .exists(),
    ...
    fn changed_files_manifest_round_trips_get_files_changed_since() {
        ...
        let resolved = crate::git::get_files_changed_since(path, "HEAD").expect("changed files");
        assert!(
            !resolved.is_empty(),

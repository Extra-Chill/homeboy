...
"#,
  ...
  "sourceUrl": "https://example.com/custom.git"
    ...
    fn is_workdir_clean_non_git_dir_returns_true() {
        ...
        assert!(
            homeboy_core::git::is_workdir_clean_or_not_git(temp.path()),
...
/// Provider that wires the extension install runner into core's
/// `component_install_provider` hook.
pub struct ExtensionComponentInstallRunner;

impl homeboy_core::component_install_provider::ComponentInstallRunner
    for ExtensionComponentInstallRunner
{
...
/// Register the extension install runner with core. Call once at startup.
pub fn register_component_install_runner() {
    homeboy_core::component_install_provider::register_component_install_runner(Box::new(
        ExtensionComponentInstallRunner,
    ));
}

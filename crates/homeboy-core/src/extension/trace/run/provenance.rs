...
pub(super) fn git_provenance(path: &Path, source: Option<&str>) -> TraceGitProvenance {
    let probe_path = crate::git::git_probe_path(path);
    let git_root = crate::git::get_git_root(&probe_path.to_string_lossy())
        .ok()
        .map(PathBuf::from)
    ...
    TraceGitProvenance {
        ...
        branch: crate::git::current_branch(&git_root),
...
fn git_stdout(path: &Path, args: &[&str]) -> Option<String> {
    crate::git::run_git(path, args, "trace provenance git probe")
        .ok()

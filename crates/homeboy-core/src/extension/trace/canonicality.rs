...
use crate::lab_workspace_provenance::{with_lab_workspace_provenance, LabWorkspaceProvenanceInfo};
#[cfg(test)]
...
pub(crate) fn trace_toolchain_provenance_requirements(
    ...
    let manifest = match crate::extension::load_extension(&context.extension_id) {
    ...
    for key in &requirement.env_keys {
        ...
        match value {
            Some(path) if !path.trim().is_empty() => checks.push(check_git_checkout(
                ...
                &crate::git::git_probe_path(Path::new(path)),
                None,
                None,
                reasons,
            )),

...
pub(super) fn checkout_deploy_tags(
    ...
    for component in components {
        ...
        let tag_sha =
            crate::engine::command::run_in_optional(path, "git", &["rev-parse", "--short", &tag]);
        ...
        let head_ahead = crate::engine::command::run_in_optional(
            path,
            "git",
...
fn current_checkout_ref(path: &str) -> String {
    crate::engine::command::run_in_optional(path, "git", &["symbolic-ref", "--short", "HEAD"])
        .or_else(|| {
            crate::engine::command::run_in_optional(path, "git", &["rev-parse", "--short", "HEAD"])
        })
        .unwrap_or_else(|| "unknown".to_string())
}

fn dirty_checkout_files(path: &str) -> Vec<String> {
    crate::engine::command::run_in_optional(path, "git", &["status", "--porcelain"])
        .map(|status| {
            status
                .lines()
                .map(str::trim)

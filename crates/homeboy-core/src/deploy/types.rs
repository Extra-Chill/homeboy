...
use crate::artifact_inputs::ResolvedArtifactInput;
use crate::component::Component;
use crate::config;
use crate::error::Result;
use crate::is_zero_u32;
use crate::paths as base_path;
use crate::phase_timing::PhaseTimingReport;
...
pub(crate) fn sha256_file(path: &Path) -> Result<String> {
    let mut file = fs::File::open(path).map_err(|error| {
        crate::error::Error::internal_io(error.to_string(), Some(path.display().to_string()))
    })?;
...
impl ComponentDeployResult {
    ...
    pub(super) fn with_source_identity(mut self, component: &Component, head_deploy: bool) -> Self {
        ...
        self.upstream_branch = git_output(path, &["rev-parse", "--abbrev-ref", "@{upstream}"])
            .or_else(|| crate::git::default_remote_branch(path));

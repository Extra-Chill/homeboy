...
pub fn reconcile_standalone_registration(
    ...
    // (#6938)
    if crate::component::local_path_is_relative(&registered_local_path) {
        status = "relative_local_path".to_string();
        if discovered_local_path.is_none() {
            if let Ok(absolute) =
                crate::component::normalize_component_local_path(&registered_local_path)
            {

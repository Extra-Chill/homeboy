...
mod tests {
    ...
    fn run_package_collects_artifacts_from_multiple_package_providers() {
        crate::test_support::with_isolated_home(|_| {
            ...
            crate::extension::save_manifest(&package_a).expect("save package A extension");
            crate::extension::save_manifest(&package_b).expect("save package B extension");
    ...
    fn git_tag_step_refuses_invalid_release_tags() {
        ...
        assert_eq!(result.status, ReleaseStepStatus::Failed);
        assert!(
            !crate::git::tag_exists_locally(&component.local_path, "v0.6.13").unwrap(),

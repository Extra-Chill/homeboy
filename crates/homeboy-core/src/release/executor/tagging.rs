...
fn inspect_release_tag_state(component: &Component, tag_name: &str) -> Result<ReleaseTagState> {
    ...
    let local_tag_commit =
        if crate::git::tag_exists_locally(&component.local_path, tag_name).unwrap_or(false) {
            Some(crate::git::get_tag_commit(&component.local_path, tag_name)?)
        } else {
            None
        };
    let remote_tag_commit = crate::git::remote_tag_commit(&component.local_path, tag_name)?;
...
fn build_existing_tag_failure(
    ...
    let mut hints = vec![crate::error::Hint {
        message: format!(

...
pub(crate) fn run_git_push(
    ...
    let current_branch = crate::git::current_branch(std::path::Path::new(&component.local_path))
        .ok_or_else(|| {
...
fn recover_advanced_remote_push(
    ...
    crate::git::fetch_origin(path)?;

    let Some(remote_commit) = crate::git::remote_branch_commit(path, branch)? else {
    ...
    let head_commit = crate::git::get_head_commit(path)?;
    ...
    if remote_commit == head_commit || crate::git::is_ancestor(path, &remote_commit, &head_commit)?
    {
...
fn push_error_message(output: &crate::git::GitOutput) -> String {
...
fn non_fast_forward_recovery_hints(component_id: &str, branch: &str) -> Vec<crate::error::Hint> {
    vec![
        crate::error::Hint {
            message: format!(

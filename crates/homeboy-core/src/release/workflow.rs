...
fn prepared_tag_publish_recovery_result(
    ...
    if !crate::git::tag_exists_locally(&release_scope.git_root, &tag)? {
        return Ok(None);
    }
    let tag_commit = crate::git::get_tag_commit(&release_scope.git_root, &tag)?;
    let head_commit = crate::git::get_head_commit(&release_scope.git_root)?;
...
fn current_component_version(component: &crate::component::Component) -> Result<Option<String>> {
...
mod tests {
    ...
    fn reconcile_release_branch_rebases_onto_advanced_remote() {
        ...
        let component = crate::component::Component {
            ...
            ..crate::component::Component::default()
        };

...
use crate::engine::command::StdoutLineObserver;
use crate::engine::shell;
use crate::error::{Error, Result};
use crate::redaction::{redact_argv, RedactionPolicy};
    ...
    let stdout_line_observer = progress_sink.as_ref().map(|sink| {
        ...
        Arc::new(move |line: &str| {
            if let Some(progress) =
                crate::runner::progress::parse_child_progress_line(line, &env, &secret_env_names)
            {
                let _ = sink(progress);
            }

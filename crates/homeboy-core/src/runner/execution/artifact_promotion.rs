...
use crate::performance_hotspots::PERFORMANCE_HOTSPOTS_SUMMARY_SCHEMA;
use crate::runners::{
...
fn remote_runner_exec_artifact_type(
    ...
    path: &Path,
) -> crate::Result<RunnerExecArtifactSourceType> {
    let path_string = path.display().to_string();
...
fn validate_runner_exec_artifact_path(runner: &runner::Runner, path: &Path) -> crate::Result<()> {
    let roots = allowed_runner_exec_artifact_roots(runner);
    match crate::paths::authorize_remote_artifact_path(
        path,
        &roots,
        crate::paths::RemotePathRootContainment::NativePath,
    ) {
        Ok(()) => Ok(()),
        Err(crate::paths::RemotePathAuthorizationError::NotAbsolute) => {
            Err(Error::validation_invalid_argument(
                "path",
        ...
        Err(crate::paths::RemotePathAuthorizationError::ContainsParentDir) => {
            Err(Error::validation_invalid_argument(
                "path",
        ...
        Err(crate::paths::RemotePathAuthorizationError::OutsideAllowedRoots) => {
            Err(Error::validation_invalid_argument(
                "path",
...
fn runner_exec_attach_download_path(runner_id: &str, path: &Path) -> crate::Result<PathBuf> {
    let file_name = path
        .file_name()

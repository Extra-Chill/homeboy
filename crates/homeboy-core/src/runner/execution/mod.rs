...
fn extension_provenance(required_extensions: &[String]) -> Vec<ExtensionProvenance> {
    let mut extensions = required_extensions
        ...
        .filter_map(|extension_id| {
            let manifest = crate::extension::load_extension(extension_id).ok()?;
            let path = manifest.extension_path.clone().unwrap_or_else(|| {
                crate::paths::extension(extension_id)
            ...
            Some(ExtensionProvenance {
                ...
                path,
                install_mode: if crate::extension::is_extension_linked(extension_id) {
                    "linked".to_string()
                } else {
                    "copied".to_string()
                },
                ...
                source_revision: crate::extension::read_source_revision(extension_id),
            })
        })
        .collect::<Vec<_>>();
...
fn persist_runner_execution_transition(
    ...
    let run = store.start_run(
        NewRunRecord::builder("runner_execution")
            .command(crate::redaction::redact_argv_display(command))
...
const RUNNER_EXEC_RUN_ID_ENV_NAMES: &[&str] = &[
    crate::observation::ACTIVE_RUN_ID_ENV,
    "HOMEBOY_RUN_ID",
...
fn runner_exec_homeboy_binaries(
    ...
    let controller_identity = crate::build_identity::current();

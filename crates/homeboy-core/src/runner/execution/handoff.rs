...
use crate::api_jobs::{Job, JobEvent};
...
pub(crate) fn lab_offload_handoff_hints(
    ...
    if let Some(run_id) = persisted_run_id.filter(|run_id| !run_id.trim().is_empty()) {
        ...
        hints.push(
            "If the command succeeded but those artifact readers show zero artifacts, promote or attach the produced output directory before using the run as review evidence; see `homeboy self docs operators/artifact-loop-runner-matrix`.".to_string(),
        );
...
pub(super) mod test_cancel_hook {
    ...
    pub(in crate::runner::execution) struct Guard;
    ...
    pub(in crate::runner::execution) fn install(hook: CancelHook) -> Guard {

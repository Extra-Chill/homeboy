...
pub fn execute_lab_offload(request: LabOffloadRequest<'_>) -> Result<LabOffloadOutcome> {
    if request.placement == homeboy_cli_contract::Placement::Auto
        && crate::resource_policy_context::is_managed_runner_placement_context()
    {
    ...
    if let crate::lab_contract::LabCommandPortability::LocalOnly(reason) = contract.portability {
        if let Some(runner_id) = request.explicit_runner {
            let message = format!(
    ...
    if !contract.routing_policy.default_lab_offload
        ...
        && contract.source_path_mode != crate::lab_contract::LabSourcePathMode::RunnerResident
        && crate::resource_policy_context::captured_context()
    ...
    let release_gate_local_hot_allowed =
        crate::defaults::resolve_release_gate_local_hot_policy().is_allowed();

...
fn lab_git_workspace_sync_uses_snapshot_for_private_proxied_sources() {
    let source_policy = crate::runner::source_materialization::SourceMaterializationPolicy {
...
fn in_flight_daemon_disconnect_outcome_marks_durable_run_detached() {
    crate::test_support::with_isolated_home(|_| {
        ...
        let record =
            crate::agent_task_lifecycle::status("run-123").expect("inspectable agent-task record");

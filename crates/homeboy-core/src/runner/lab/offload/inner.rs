...
use crate::secret_env_plan::SECRET_ENV_PLAN_ENV_DELTA_SOURCE;
...
pub(crate) fn run_lab_offload_inner(
    ...
    let provider_rotation =
        serde_json::to_value(crate::defaults::load_config().agent_task.rotation)
    ...
    workspace_resource_lifecycle.cleanup_policy = if agent_task_run_id.is_some() {
        crate::resource_lifecycle_index::ResourceCleanupPolicy::Preserve
    } else {
...
fn job_artifact_is_fuzz_result(artifact: &crate::api_jobs::JobArtifactMetadata) -> bool {
    artifact
        .name
        .as_deref()
...
fn runner_artifact_is_fuzz_result(artifact: &crate::runner::RunnerArtifactRef) -> bool {
    artifact
        .name
        .as_deref()
        .is_some_and(artifact_name_or_kind_is_fuzz_result)

...
pub(crate) fn rig_declared_path_input_extra_workspaces(
    args: &[String],
    workload: Option<&crate::lab_contract::LabRigWorkloadArguments>,
    primary_source_path: &Path,
) -> Result<Vec<ExtraLabWorkspace>> {
    if !workload.is_some_and(|workload| {
        matches!(
            workload.kind,
            crate::lab_contract::LabRigWorkloadKind::Bench
        )
    }) {
        return Ok(Vec::new());
    }
...
mod tests {
    ...
    fn planner_combines_provider_settings_and_rig_passthrough_inputs_before_staging() {
        ...
        let mut contract = LabOffloadCommand {
            command: crate::lab_contract::LabCommandContract::portable("bench", None, false, &[]),
            required_extensions: Vec::new(),
            required_capabilities: Vec::new(),
            workload: None,
        };
        contract.workload = Some(crate::lab_contract::LabRigWorkloadArguments {
            kind: crate::lab_contract::LabRigWorkloadKind::Bench,

...
use crate::agent_tasks::scheduler::AgentTaskPlan;
use crate::agent_tasks::secrets as agent_task_secrets;
use crate::agent_tasks::{
    AgentTaskExecutor, AgentTaskRequest, AgentTaskWorkspace, AGENT_TASK_REQUEST_SCHEMA,
};
use crate::lab_contract::LabSecretEnvSource;
...
pub(super) fn build_lab_secret_env_handoff_plan(
    ...
    let trace_secret_env = if secret_env_sources.contains(&LabSecretEnvSource::Trace) {
        ...
        crate::trace_secrets::empty_status()
    };
...
pub(super) fn hydrate_trace_secret_env(
    ...
    if names.is_empty() {
        return Ok(crate::trace_secrets::empty_status());
    }

    let project_id = trace_project_id_from_args(args);
    let (resolved, statuses) =
        crate::trace_secrets::resolve_secret_env(&names, project_id.as_deref())?;
    for (name, value) in resolved {
        env.insert(name, value);
    }

    Ok(crate::trace_secrets::status_metadata(statuses))
}
...
fn declared_agent_task_controller_secret_sources_with_providers(
    ...
    providers: &[AgentTaskExecutorProvider],
) -> Result<HashMap<String, crate::defaults::AgentTaskSecretSource>> {

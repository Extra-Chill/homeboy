...
fn terminate_unpersisted_child_and_reap(child: &mut std::process::Child) -> Result<()> {
    let pid = child.id();
    let group = crate::process::terminate_isolated_process_group(pid);
    let tree = crate::process::terminate_process_tree_best_effort(pid);
...
fn resolved_resource_guard_limits(
    ...
    let memory_capacity_bytes =
        crate::resources::memory::probe_system_memory().map(|memory| memory.total_bytes);
...
mod tests {
    ...
    fn child_started_failure_kills_and_reaps_the_spawned_child() {
        ...
        assert!(!crate::process::pid_is_running(pid.load(Ordering::SeqCst)));
    }

...
pub(in crate::runner) struct ExecutionEnvelope {
    argv: Vec<String>,
    pub inputs: LabCommandInputs,
}

#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub(in crate::runner) struct LabCommandInputs {
    pub provider_configs: Vec<ArgRef>,
    pub agent_task_text_specs: Vec<ArgRef>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::runner) struct ArgRef {
    pub flag: &'static str,
    pub value: ArgValue,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::runner) enum ArgValue {
    InlineText(String),
    PathRef(String),
    Stdin,
    Missing,
...
impl ExecutionEnvelope {
    ...
    pub fn try_rewrite_provider_config_values(
        &self,
        mut rewrite: impl FnMut(&str) -> crate::Result<String>,
    ...
    pub fn rewrite_agent_task_text_values(
        &self,
        mut rewrite: impl FnMut(&str, &str) -> crate::Result<String>,
    ) -> crate::Result<Vec<String>> {
        try_rewrite_flag_value_args(&self.argv, |arg, iter, out| {
            if let Some(flag) = agent_task_text_flag(arg) {
                out.push(arg.to_string());

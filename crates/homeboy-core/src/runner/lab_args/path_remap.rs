...
#[derive(Debug, Clone)]
pub(in crate::runner) struct LabPathRemap {
    pub local: String,
    pub remote: String,
}

impl From<crate::runner_execution_envelope::PathMaterializationPathRemap> for LabPathRemap {
    fn from(remap: crate::runner_execution_envelope::PathMaterializationPathRemap) -> Self {
        Self {
...
where
    F: FnMut(&str, &mut Peekable<Iter<'_, String>>, &mut Vec<String>) -> crate::Result<()>,
{
    let mut out = Vec::with_capacity(args.len());
    let mut iter = args.iter().peekable();
    let mut passthrough = false;
...
pub(in crate::runner) fn remap_path_settings_in_args(
    args: &[String],

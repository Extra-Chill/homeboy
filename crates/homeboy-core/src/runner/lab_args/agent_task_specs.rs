...
pub(in crate::runner) struct AgentTaskSpecMaterialization<T> {
    pub(in crate::runner) argv: Vec<String>,
    pub(in crate::runner) workspace_entries: Vec<AgentTaskSpecWorkspaceEntry<T>>,
}

pub(in crate::runner) struct AgentTaskSpecWorkspaceEntry<T> {
    pub(in crate::runner) step_id: &'static str,
    pub(in crate::runner) entry: T,
}

pub(in crate::runner) struct AgentTaskInlineJsonSpec<'a> {
    pub(in crate::runner) spec: &'a str,
    pub(in crate::runner) filename: &'static str,
    pub(in crate::runner) role: &'static str,
}

pub(in crate::runner) fn materialize_agent_task_specs_in_args<T>(
    args: &[String],
...
pub(in crate::runner) fn materialize_inline_agent_task_json_specs_in_args<T>(
    args: &[String],
    mut sync_inline_json: impl FnMut(AgentTaskInlineJsonSpec<'_>) -> Result<Option<(String, T)>>,
...
pub(in crate::runner) fn remap_agent_task_plan_in_args(
    args: &[String],
    mappings: &[LabPathRemap],
    source_path: &Path,
) -> Result<Vec<String>> {
...
pub(in crate::runner) fn inline_agent_task_prompt_files_in_args(
    args: &[String],
    source_path: &Path,
) -> Result<Vec<String>> {
...
fn rewrite_fanout_cook_workspaces(
    ...
    for (index, cook) in cooks.iter_mut().enumerate() {
        ...
        for field in ["cwd", "workspace"] {
            ...
            let resolved_controller_path = crate::worktree::resolve(&controller_value)
                .ok()

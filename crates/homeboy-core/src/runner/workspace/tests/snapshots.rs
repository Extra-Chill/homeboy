...
use super::git;
use crate::runner::workspace::sync::{sync_workspace, workspace_snapshots};
use crate::runner::workspace::types::{
...
fn workspace_snapshots_render_metadata_for_synced_workspace() {
    crate::test_support::with_isolated_home(|_| {
        ...
        let commit = crate::runner::workspace::util::git_output(&source, &["rev-parse", "HEAD"])
            .expect("commit");
...
fn workspace_snapshots_filters_by_repo_ref_commit_and_run() {
    crate::test_support::with_isolated_home(|_| {
        ...
        let first_commit =
            crate::runner::workspace::util::git_output(first.path(), &["rev-parse", "HEAD"])
                .expect("first commit");

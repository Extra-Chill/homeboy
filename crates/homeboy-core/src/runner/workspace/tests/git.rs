...
fn changed_since_retry_route_materializes_private_unavailable_origin_from_bundle() {
    crate::test_support::with_isolated_home(|_| {
        ...
        let changed_since =
            crate::runner::prepare_git_lab_offload_changed_since(&args, source.path())
...
fn git_sync_of_detached_extension_source_preserves_source_revision() {
    crate::test_support::with_isolated_home(|_| {
        ...
        let install = crate::extension::install(
        ...
        assert_eq!(
            crate::extension::read_source_revision("wordpress").as_deref(),
            Some(detached_short.as_str())
        );
    });
}

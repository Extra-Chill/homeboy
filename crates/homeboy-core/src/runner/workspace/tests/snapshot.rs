...
fn snapshot_git_readback_failure_fails_materialization_contract() {
    let runner: crate::runner::Runner = serde_json::from_value(serde_json::json!({
        "id": "lab", "kind": "local"
    }))
    .expect("local runner");
...
use crate::runner::workspace::sync::{list_workspaces, sync_workspace};
...
fn copy_snapshot_materializes_symlinked_dependency_contents() {
    ...
    crate::runner::workspace::snapshot::copy_snapshot_to_directory(&source, &destination, &[])

...
use crate::{git::clone_repo, paths};
...
fn prepare_validation_dependency_workspace(
    ...
    crate::hygiene::write_validation_dependency_source_evidence(
        &component.id,
        &path,
        &prepared_path,
    )?;
...
fn prepare_dependency_git_state(component: &Component, path: &Path) -> Result<()> {
    crate::hygiene::require_checkout_hygiene_without_lifecycle(
        vec![crate::hygiene::DependencyCheckout {
            id: component.id.clone(),
            role: "validation_dependency".to_string(),
            path: path.to_path_buf(),
        }],
        crate::hygiene::DependencyHygieneOptions { allow_stale: false },
    )?;
    Ok(())
}

fn run_dependency_lifecycle(component: &Component, path: &Path) -> Result<()> {
    crate::hygiene::run_validation_dependency_lifecycle(component, path)
}
...
mod tests {
    ...
    fn sync_workspace_clones_missing_registered_validation_dependency() {
        crate::test_support::with_isolated_home(|_| {
            ...
            let components_dir = crate::paths::components().expect("components dir");

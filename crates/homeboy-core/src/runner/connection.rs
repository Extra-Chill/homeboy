...
pub fn connect_with_orphan_adoption(
    ...
    let promotion_lease =
        crate::runtime_promotion::acquire("runner daemon reconnect", runner_id.to_string())?;
...
>
where
    Probe: FnOnce() -> crate::server::CommandOutput,
    Recover: FnOnce(RunnerLeaselessRecoveryContract) -> crate::server::CommandOutput,
{
    let probe = probe();
...
fn leaseless_recovery_failure_message(output: &crate::server::CommandOutput) -> String {
    if output.timed_out {
        format!(
...
fn state_loss_recovery_failure_message(output: &crate::server::CommandOutput) -> String {
    if output.timed_out {
        format!(
...
pub fn connect_reverse(options: ReverseRunnerConnectOptions) -> Result<(RunnerConnectReport, i32)> {
    ...
    let homeboy_identity = crate::build_identity::current();
...
fn runner_daemon_freshness(
    ...
    connected: bool,
) -> Result<Option<crate::daemon::DaemonFreshnessReport>> {

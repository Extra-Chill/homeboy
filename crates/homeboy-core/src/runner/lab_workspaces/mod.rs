...
use crate::worktree::TaskWorktreeState;
use crate::{agent_task_provider, component, worktree, Error, Result};
...
pub(super) fn provider_config_extra_workspaces(
    ...
    for spec in provider_config_specs(args) {
        let raw = match crate::config::read_json_spec_to_string(&spec) {
            Ok(raw) => raw,
            Err(_) => continue,
        };
...
fn read_agent_task_plan_spec_to_string(spec: &str, source_path: &Path) -> Result<String> {
    let Some(_) = spec.strip_prefix('@') else {
        return crate::config::read_json_spec_to_string(spec);
    };

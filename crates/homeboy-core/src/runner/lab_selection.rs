...
pub(super) fn resolve_lab_runner_selection(
    ...
    let release_gate_local_hot_allowed =
        crate::defaults::resolve_release_gate_local_hot_policy_from(&config).is_allowed();
...
fn fail_if_local_bench_denied(command: &LabOffloadCommand, denied: bool) -> Result<()> {
    ...
    let config_path =
        crate::defaults::config_path().unwrap_or_else(|_| "the global Homeboy config".to_string());
...
pub(super) fn release_gate_local_hot_denied_error(message: String, field: &str) -> Error {
    let config_path =
        crate::defaults::config_path().unwrap_or_else(|_| "the global Homeboy config".to_string());
    let env_var = crate::defaults::RELEASE_GATE_LOCAL_HOT_ENV;

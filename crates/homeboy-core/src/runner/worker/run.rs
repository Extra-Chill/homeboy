...
use crate::api_jobs::RemoteRunnerJobResult;
...
pub fn run_reverse_worker(
    ...
    if options.loop_mode {
        crate::process::install_shutdown_handler(stop.clone(), "runner worker")?;
    }
...
fn run_once_output(
    ...
    let poll_cancelled =
        |client: &Client, options: &ReverseRunnerWorkerOptions, job: &crate::api_jobs::Job| {
...
fn runner_exec_options_from_envelope(
    ...
    capability_preflight: Option<crate::runner::RunnerCapabilityPreflight>,

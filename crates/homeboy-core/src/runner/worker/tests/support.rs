...
use crate::api_jobs::{JobEventKind, JobStore, RemoteRunnerJobResult};
...
pub(super) fn write_reverse_controller_session(broker_url: &str) {
    let path = crate::paths::runner_session_file("lab").expect("session path");
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).expect("create session dir");
    }
    let session = crate::runner::RunnerSession {
        runner_id: "lab".to_string(),
        mode: crate::runner::RunnerTunnelMode::Reverse,

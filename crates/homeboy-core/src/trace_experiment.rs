...
use crate::extension::trace as extension_trace;
use crate::rig;
...
/// needs.
pub fn prepare_experiment_bundle_dir(dir: &Path) -> crate::Result<()> {
...
/// `mkdir -p`.
pub fn prepare_experiment_overlay_dir(dir: &Path) -> crate::Result<()> {
    std::fs::create_dir_all(dir).map_err(|err| {
        crate::Error::internal_io(
            format!(
...
pub fn write_experiment_file(path: &Path, content: &str, context: &str) -> crate::Result<()> {
...
pub fn write_experiment_json_file<T: Serialize>(
    ...
    let content = serde_json::to_string_pretty(value)
        .map_err(|err| crate::Error::internal_json(err.to_string(), Some(context.to_string())))?;
    write_experiment_file(path, &(content + "\n"), context)
}

/// Read the raw bytes of a trace-experiment overlay source file for bundling.
pub fn read_experiment_overlay(source: &Path) -> crate::Result<Vec<u8>> {
...
pub fn write_experiment_overlay(target: &Path, bytes: &[u8]) -> crate::Result<()> {
...
pub fn read_experiment_overlay_for_checksum(path: &Path) -> crate::Result<Vec<u8>> {

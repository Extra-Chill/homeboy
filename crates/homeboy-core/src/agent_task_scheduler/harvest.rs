...
fn attempt_patch_path(running: &RunningTask, kind: &str) -> Result<PathBuf, HarvestError> {
    ...
    let dir = crate::artifacts::root()
        ...
        .join(crate::paths::sanitize_path_segment(&running.task_id));
...
fn attempt_patch_id(running: &RunningTask, kind: &str) -> String {
    format!(
        "{}-attempt-{}-{kind}",
        crate::paths::sanitize_path_segment(&running.task_id),
        running.attempt
    )
}
...
mod committed_harvest_tests {
    ...
    fn authorized_dirty_lab_snapshot_preflight_materializes_a_provider_ready_attempt_workspace() {
        ...
        let content_hash =
            crate::runner::workspace_content_hash(workspace.path(), &snapshot.sync_excludes)
                .expect("content hash");
    ...
    fn lab_snapshot_git_preflight_accepts_the_production_synthetic_checkout() {
        ...
        crate::test_support::with_isolated_home(|_| {
            ...
            let content_hash =
                crate::runner::workspace_content_hash(&remote, &snapshot.sync_excludes)
    ...
    fn controller_context_ignores_ambient_lab_signal_without_leaking_it_to_local_cells() {
        ...
        assert!(!workspace.path().join(".git").exists());
        assert!(std::env::var(crate::observation::LAB_OFFLOAD_METADATA_ENV).is_ok());
        std::env::remove_var(crate::observation::LAB_OFFLOAD_METADATA_ENV);
    }

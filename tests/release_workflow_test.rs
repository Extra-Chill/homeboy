...
#[test]
fn release_recovery_keeps_control_revision_in_the_owning_finalizer() {
    let workflow = release_workflow();
    let host = job_section(workflow, "host");

    assert!(
        !host.contains("name: Gate publication on the declared asset set"),
        "an absent release has no remote inventory to inspect before the finalizer creates its draft"
    );
    assert!(
        !host.contains("release-asset-completeness.sh"),
        "remote inventory validation must stay inside the finalizer that owns draft creation and publication"
    );
    let adoption = release_step_block(host, "name: Create remote draft adoption manifest");
    assert!(
        adoption.contains("CONTROL_SHA: ${{ github.sha }}"),
        "recovery must still bind the owning finalizer to the workflow control revision"
    );
}

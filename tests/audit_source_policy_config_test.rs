...
/// Resolve the rows a baseline policy section covers.
///
/// A section states its membership either as a `fingerprint_prefix` predicate
/// evaluated against the baseline's own `known_fingerprints` (the current form)
/// or as an explicit `known_fingerprints` enumeration (the legacy form, kept
/// readable so pre-migration baselines still load). Both are honored here for
/// the same reason `AuditBaselinePolicySection::effective_fingerprints` honors
/// both: the assertion is about the effective set, not the serialized shape.
fn policy_section_rows<'a>(config: &'a Value, section: &'a Value) -> Vec<&'a str> {
    let mut rows = section["known_fingerprints"]
        .as_array()
        .map(|rows| rows.iter().filter_map(|row| row.as_str()).collect())
        .unwrap_or_else(Vec::new);

    if let Some(prefix) = section["fingerprint_prefix"].as_str() {
        rows.extend(
            config["baselines"]["audit"]["known_fingerprints"]
                .as_array()
                .expect("baseline known_fingerprints is an array")
                .iter()
...
#[test]
fn baseline_policy_section_membership_is_derived_not_re_listed() {
    // The section used to be a byte-for-byte re-listing of rows already present
    // in the top-level array — 678 of them, ~24% of the file — regenerated on
    // every baseline save. Enumerating derivable data is the debt; pinning the
    // predicate keeps it from coming back on the next `--update-baseline`.
    ...
    assert_eq!(
        section["fingerprint_prefix"].as_str(),
        Some("core_boundary_leak:core-agnostic-source"),
        "the section must state the predicate that selects its rows"
    );
    // The key stays present and empty rather than absent: a homeboy binary
    // older than `known_fingerprints`' `#[serde(default)]` still reads this
    // file (homeboy-action falls back to the last release when a source build
    // fails), and a missing key fails its deserialization of the whole
    // baseline. Empty resolves to the same effective set on both binaries.
    assert_eq!(
        section["known_fingerprints"].as_array().map(Vec::len),
        Some(0),
        "the section must not re-list rows the baseline already carries"
    );
    assert!(
        !policy_section_rows(&config, section).is_empty(),
        "the predicate must still resolve to real baseline rows"
    );
}
...
fn new_term_findings_are_covered_by_the_baseline_policy_section() {
    ...
    let section = config["baselines"]["audit"]["metadata"]["policy_sections"]
        ...
        .find(|section| section["audit_policy"] == "core_boundary_leak:core-agnostic-source")
        .expect("core-agnostic-source has a baseline policy section");
    let rows = policy_section_rows(&config, section);

...
| --- | --- | --- |
| `HOMEBOY_CONFIG_ROOT` env var; the process-local home-root override (`set_home_root_override`); `$HOME`/`APPDATA`. | First match wins, non-Windows: (1) `HOMEBOY_CONFIG_ROOT`, if set to a non-empty value, is used *verbatim as the complete config directory* -- it replaces `~/.config/homeboy` entirely, not just `~`. This is the explicit lever for CI and multi-config operators, and it does not require swapping `HOME`. (2) The process-local home-root override, when set -- `<override>/.config/homeboy`. (3) `$HOME/.config/homeboy` -- the historical default. `XDG_CONFIG_HOME` is deliberately not consulted: an inherited runner export cannot be told apart from an intentional relocation, and each rule tried (#14784 honor-always, #14794 honor-under-HOME) broke a different isolation contract (hermetic tests leaking into the runner's real config; the agent-task gate's out-of-home XDG relocation being discarded). `HOMEBOY_CONFIG_ROOT` is explicit and is the only override. On Windows, `HOMEBOY_CONFIG_ROOT` is still honored first for parity, then `APPDATA/homeboy`. | All tiers are implemented in `homeboy()` in `crates/homeboy-paths/src/lib.rs`; every other config path (`homeboy_json()`, `extension()`, `key()`, rig/runner state, etc.) derives from it, so this is the single place the precedence is enforced. |
...
| Locations | Effective precedence | Resolving code path |
| --- | --- | --- |
| Component `hooks`; project/fleet component override `hooks`; project-level `Project::hooks`; extension manifest `hooks`; release/deploy lifecycle callers. | For a component-scoped event (e.g. `post:deploy`): fleet/project component overrides replace the component's whole `hooks` map when non-empty (project wins over fleet) — this is the base component `hooks` layer used below. Resolution then merges, in order: (1) extension hooks (platform behavior), (2) the project's own `Project::hooks` (site policy, homeboy#14973), (3) the resolved component `hooks` (customization). `HookEvent::PostDeployProject` is a separate, project-scoped event: it is declared only on `Project::hooks`, runs once per deploy invocation after the last component, and is never merged from extensions or components. | Component hooks are declared in `crates/contracts/homeboy-component-contract/src/model.rs`. Project/fleet component-override replacement is in `crates/homeboy-core/src/project/component/overrides.rs`. `Project::hooks` is declared in `crates/homeboy-core/src/project/mod.rs`. The three-way merge and the project-scoped-event rationale are in `crates/homeboy-core/src/engine/hooks.rs` (`resolve_hooks_with_extensions_and_project`, `run_project_scoped_hooks_remote`). The project-scoped `post:deploy:project` call site is `crates/homeboy-deploy/src/orchestration/project_hooks.rs`. |

...
> **Status.** This is a point-in-time investigation, not a maintained contract. It cites files rather than line ranges: the original `path.rs:LINE` citations all drifted and several files moved out of `homeboy-core` during the crate extraction (component model and config to `crates/contracts/homeboy-component-contract/`, extension code to `crates/homeboy-extension/`, deploy path roots to `crates/homeboy-release/`, rig spec to `crates/homeboy-rig/`, runner loading to `crates/homeboy-lab-runner/`, tunnel entity to `crates/homeboy-tunnel/`). Only the consolidation in "Consolidation Shipped In This Slice" shipped; none of the six removals in "Simplification Proposal" have been implemented, and the five "Ambiguous Or Order-Dependent Areas" remain untraced. Treat both sections as an open backlog, and re-verify against the tree before relying on any claim here.
...
| Extension manifest | `ExtensionManifest` in `crates/homeboy-extension/src/manifest.rs` |
...
| Extension capability ownership | `resolve_extension_for_capability()` errors when multiple linked extensions support the same capability in `crates/homeboy-extension/src/capability.rs`; there is no precedence tie-breaker. |

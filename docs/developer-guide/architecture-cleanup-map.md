...
| Deprecated `homeboy file upload` | `src/commands/file/mod.rs` | Deprecated command path that points callers to `file copy`. | Remove after scripts/docs use `file copy` and no compatibility issue remains open. |

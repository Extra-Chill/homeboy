...
`homeboy` exposes 569 visible commands across 40 top-level command families. Every page below is generated from the clap command tree in `crates/homeboy-cli`, so it cannot drift from the binary.
...
| `homeboy topology` | [topology.md](topology.md) | Inspect declared resource relationships without resolving effective configuration |

...
| `--provider-evidence` | `<JSON>` | Read-only external file streamed into a private, digest-addressed projection in `.homeboy/evidence/<id>/` (64 MiB maximum per file). Repeat for each source the provider may read |

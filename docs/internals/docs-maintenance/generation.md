...
Use `homeboy self docs map <component>` to generate a codebase map for AI-assisted documentation, then create files manually following the structure standards in `homeboy self docs internals/docs-maintenance/structure`.
...
- For directory entry points, use `{directory}/{directory}.md` pattern (see `homeboy self docs internals/docs-maintenance/structure`)

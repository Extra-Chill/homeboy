...
`homeboy runs show <run-id>` prints a compact human summary by default: run identity, status, component/rig/SHA, timestamps, bench hotspots when present, and each recorded artifact's locator with a concise `homeboy runs artifact get <run-id> <artifact-id>` command to inspect it. This makes bench artifacts (shared-state files, WP Codebox bundles, scenario outputs) and slow/hot metric families easy to find without spelunking temp directories. Pass `--json` for the full structured payload on stdout; it is also always written to `--output <file>`.

...
`homeboy runs show <run-id>` prints a compact human summary by default: run identity, status, component/rig/SHA, timestamps, bench hotspots and generic coverage summaries when present, and each recorded artifact's locator with a concise `homeboy runs artifact get <run-id> <artifact-id>` command to inspect it. This makes bench artifacts, scenario outputs, slow/hot metric families, and fuzzer/bench coverage gaps easy to find without spelunking temp directories. Coverage rendering is schema-blind and only uses generic metadata fields such as `coverage_summary`, `coverage_gaps`, `surface_count`, `exercised_count`, `skipped_count`, and `failed_count`. Pass `--json` for the full structured payload on stdout; it is also always written to `--output <file>`.

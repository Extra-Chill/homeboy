...
`--head --package-only --tag <TAG> --apply` is for operator recovery when the release tag already exists but its release assets need to be regenerated. It requires the checked-out `HEAD` to match the existing tag, runs the component build and extension-owned `release.package` actions, copies every release asset into Homeboy's artifact root under `release-packages/<component>/<tag>/`, writes `manifest.json`, and prints both paths. The manifest is a durable inventory accepted by `release --head --from-artifacts <dir>` for later finalization. This recovery does not create tags, push history, create GitHub Releases, publish registries, deploy, or clean up the component build output.

...
`--protected-branch` prepares and pushes `release/<tag>`, then opens the release PR without creating a tag. If PR creation fails after the branch is pushed, rerun the same prepare command from that branch: `homeboy release <component_id> --protected-branch --apply`. Homeboy verifies that the checked-out branch matches the version-derived release branch, its remote ref, and the expected release commit before retrying only the branch/PR boundary. After the PR is normally merged, check out the merged default branch and run `homeboy release <component_id> --protected-branch --head --apply` to tag and publish.

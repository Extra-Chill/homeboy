# Operations

Operations docs are runbooks for people operating Homeboy workflows, runners, and evidence paths. They are more specific than workflows and may include environment repair, proof rules, or setup procedures.

## Runbooks

- [Release-gate proof path](release-gate-proof-path.md)
- [Controller to runner reverse-runner setup](controller-runner-reverse-runner.md)
- [Artifact loop for runner and matrix workflows](artifact-loop-runner-matrix.md)
- [Required CI gates](required-ci-gates.md)

## Related Workflows

- [Review a branch](../workflows/review-a-branch.md)
- [Capture evidence](../workflows/capture-evidence.md)
- [Use runners](../workflows/use-runners.md)
